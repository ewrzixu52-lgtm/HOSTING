...
=====================================================================
 ...
 AI Brain: OpenRouter (DeepSeek v3.1) - keys added via /apisetup in DM, DB-backed multi-key rotation
...
from telegram.error import TelegramError, BadRequest, Conflict as TelegramConflictError, NetworkError, TimedOut, Forbidden
...
def short_name(full_name):
    """Just the first word of someone's display name -- their Telegram
    first_name can be a whole multi-word phrase (people set weird/funny
    full 'names'), which reads clunky and un-punchy inside a short roast
    line. Falls back to the full string if it's empty/unsplittable."""
    if not full_name:
        return full_name
...
def fancy(text: str) -> str:
    ...
    owner's custom bold font (case preserved). Leaves emojis, spaces,
    punctuation, and non-Latin characters untouched."""
    if not text:
        return text
    out = []
    for ch in text:
        ...
        if low in _FANCY_LETTERS:
            out.append(_FANCY_UPPER[low] if ch.isupper() else _FANCY_LETTERS[low])
...
_custom_emoji_cache_by_char = {}   # placeholder_char -> [(name, custom_emoji_id), ...] -- every saved
                                    # premium emoji that shares this fallback glyph, so rendering can
                                    # pick a random one each time instead of always the same emoji.
...
def _refresh_custom_emoji_cache():
    ...
    for r in rows:
        ...
        if r["placeholder_char"]:
            by_char.setdefault(r["placeholder_char"], []).append((r["name"], r["custom_emoji_id"]))
...
def render_with_emojis(html_text: str, fancy_enabled: bool = True) -> tuple:
    ...
    def emit_text(chunk, apply_fancy):
        ...
        for m in emoji_pattern.finditer(chunk):
            ...
            if gd.get("ename"):
                ...
                echar = gd["echar"]
                # Multiple premium emoji can share the same fallback glyph
                # (e.g. several saved "fire" variants) -- pick a random one
                # each time so the same emoji doesn't show up every single
                # message.
                _, eid = random.choice(_custom_emoji_cache_by_char[echar])
                handled = True
            if handled:
                out.append(echar)
    ...
    for tok in tokens:
        if tok.startswith("<") and tok.endswith(">"):
            ...
            apply_fancy = fancy_enabled and not any(o["tag"] in ("a", "code", "pre") for o in open_stack)
...
# 👇 AI provider: OpenRouter (DeepSeek v3.1). NO key lives in an env var or
# in this code anymore -- keys are entered through the bot itself, in DM,
# via /apisetup, and stored in the database (ai_api_keys table). Send
# /apisetup to the bot in DM (bot owner only) to add a key; run it again to
# add another one -- as many as you want. They're used ONE AT A TIME: the
# bot sticks to a key until it actually fails (auth error / exhausted /
# down), and only then moves on to the next one -- never round-robins on
# every call.
AI_MODEL = "deepseek/deepseek-chat-v3.1"   # swap to "deepseek/deepseek-chat-v3.1:free" if you're on free-tier keys
...
OWNER_CREDIT_BUTTON_EMOJI_ID = "6129589862413638401"  # left-of-text icon on the owner-credit button
...
BACKUP_CHANNEL_ID = "-1004475152675"   # DB-backup channel -- fixed, no need to set via env anymore
...
_owner_display_name_cache = None


async def get_owner_display_name(context: ContextTypes.DEFAULT_TYPE) -> str:
    """Bot owner's current Telegram first name -- fetched once and cached
    for the life of the process, used as the label on the owner-credit
    button attached under admin-command responses."""
    global _owner_display_name_cache
    if _owner_display_name_cache is None:
        try:
            owner_chat = await context.bot.get_chat(OWNER_ID)
            _owner_display_name_cache = owner_chat.first_name or OWNER_USERNAME
        except TelegramError:
            _owner_display_name_cache = OWNER_USERNAME
    return _owner_display_name_cache


async def owner_credit_button(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    """Green owner-credit button shown under admin-command responses.
    style='success' + icon_custom_emoji_id are real Bot API 9.4 fields
    (python-telegram-bot 22.7+) -- NOT decorative HTML, this actually
    renders as a green button with a real premium-emoji icon in supported
    clients. NOTE: Telegram only exposes ONE icon slot per button, always
    to the LEFT of the text -- there's no right-side icon field, so only
    the first emoji id can go on as a real icon here."""
    name = await get_owner_display_name(context)
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(
            f"{name} 👑",
            url=f"tg://user?id={OWNER_ID}",
            style=BTN_GREEN,
            icon_custom_emoji_id=OWNER_CREDIT_BUTTON_EMOJI_ID,
        )
    ]])
...
def db_init():
    ...
    cur.executescript(
        ...
        CREATE TABLE IF NOT EXISTS group_settings (
            ...
            rules_text TEXT,
            welcome_profile_pic_enabled INTEGER DEFAULT 1,
            warn_limit INTEGER DEFAULT 5,
            warn_action TEXT DEFAULT 'ban',
            abuse_mute_min INTEGER DEFAULT 30,
            abuse_warn_enabled INTEGER DEFAULT 1,
            link_mute_min INTEGER DEFAULT 0,
            link_warn_enabled INTEGER DEFAULT 1
        );
        ...
        CREATE TABLE IF NOT EXISTS promoted_by (
            chat_id INTEGER,
    ...
    # Safe column migrations for deployments that already had group_settings
    # before these fields existed (CREATE TABLE IF NOT EXISTS doesn't alter
    # an existing table's columns).
    existing_cols = {row["name"] for row in cur.execute("PRAGMA table_info(group_settings)").fetchall()}
    for col, coltype in [
        ("welcome_enabled", "INTEGER DEFAULT 1"),
        ("welcome_media_file_id", "TEXT"),
        ("welcome_media_type", "TEXT"),
        ...
        if col not in existing_cols:
            cur.execute(f"ALTER TABLE group_settings ADD COLUMN {col} {coltype}")
...
def db_export_json() -> str:
    ...
    for table in [
        ...
        "approved_groups", "known_groups", "promoted_by", "blacklist_words",
    ]:
...
def db_import_json(raw: str) -> tuple[bool, str]:
    ...
    try:
        conn = db_connect()
        cur = conn.cursor()
        for t in required_tables + [t for t in ("group_access", "group_settings", "filters", "custom_emojis", "approved_groups", "known_groups", "promoted_by", "blacklist_words") if t in data]:
        ...
        return False, f"❌ <b>Import ke dauran error aaya: <code>{e}</code></b>\nDatabase change nahi kiya gaya (safe rollback)."
...
def get_group_owner_contact(chat_id):
    """Returns (button_label, url) for this group's designated owner --
    whoever was most recently /grant-ed access to this chat, else falls
    back to the bot's own owner (for the main/ungranted group)."""
    conn = db_connect()
    row = conn.execute(
        "SELECT user_id FROM group_access WHERE chat_id=? ORDER BY granted_at DESC LIMIT 1", (chat_id,)
    ).fetchone()
    if row:
        u = conn.execute("SELECT username, first_name FROM users WHERE user_id=?", (row["user_id"],)).fetchone()
        conn.close()
        if u and u["username"]:
            return f"@{u['username']}", f"https://t.me/{u['username']}"
        label = u["first_name"] if u and u["first_name"] else "Group Owner"
        return label, f"tg://user?id={row['user_id']}"
    conn.close()
    return "Main / Trusted Escrower", f"https://t.me/{OWNER_USERNAME}"


def add_blacklist_word(chat_id, word, added_by):
    conn = db_connect()
    try:
        conn.execute("INSERT OR IGNORE INTO blacklist_words (chat_id, word, added_by) VALUES (?,?,?)", (chat_id, word.lower(), added_by))
        conn.commit()
    finally:
        conn.close()
...
def remove_blacklist_word(chat_id, word):
    conn = db_connect()
    try:
        cur = conn.execute("DELETE FROM blacklist_words WHERE chat_id=? AND word=?", (chat_id, word.lower()))
        conn.commit()
...
def get_blacklist_words(chat_id):
    conn = db_connect()
    try:
        rows = conn.execute("SELECT word FROM blacklist_words WHERE chat_id=?", (chat_id,)).fetchall()
        return [r["word"] for r in rows]
    finally:
        conn.close()


def text_has_blacklisted_word(chat_id, text):
...
def get_warn_settings(chat_id):
    ...
    return gs.get("warn_limit") or 5, gs.get("warn_action") or "ban"


def get_abuse_link_settings(chat_id):
    """Per-group overrides for what happens on abuse / unauthorized links --
    mute duration for each (0 = mute disabled) and whether a warn is applied
    on top. Falls back to the original fixed defaults if a group hasn't
    customized these (ABUSE_MUTE_MIN minutes + warn for abuse, no mute + warn
    for links -- same behavior as before this was configurable)."""
    gs = get_group_settings(chat_id)
    abuse_mute = gs.get("abuse_mute_min")
    abuse_mute = ABUSE_MUTE_MIN if abuse_mute is None else abuse_mute
    abuse_warn = gs.get("abuse_warn_enabled")
    abuse_warn = True if abuse_warn is None else bool(abuse_warn)
    link_mute = gs.get("link_mute_min") or 0
    link_warn = gs.get("link_warn_enabled")
    link_warn = True if link_warn is None else bool(link_warn)
    return abuse_mute, abuse_warn, link_mute, link_warn


def record_promotion(chat_id, target_user_id, promoted_by_user_id):
    conn = db_connect()
    conn.execute(
        "INSERT OR REPLACE INTO promoted_by (chat_id, target_user_id, promoted_by_user_id, promoted_at) VALUES (?,?,?,?)",
        (chat_id, target_user_id, promoted_by_user_id, datetime.now(IST).isoformat()),
    )
    conn.commit()
    conn.close()


def get_promoter(chat_id, target_user_id):
    conn = db_connect()
    row = conn.execute(
        "SELECT promoted_by_user_id FROM promoted_by WHERE chat_id=? AND target_user_id=?", (chat_id, target_user_id)
    ).fetchone()
    conn.close()
    return row["promoted_by_user_id"] if row else None


def clear_promotion_record(chat_id, target_user_id):
    conn = db_connect()
    conn.execute("DELETE FROM promoted_by WHERE chat_id=? AND target_user_id=?", (chat_id, target_user_id))
    conn.commit()
    conn.close()


async def is_full_group_owner(bot, chat_id, user_id) -> bool:
    """True for: the bot's own owner, the REAL Telegram group creator, or
    whoever was /grant-ed this specific group -- these three can do
    ANYTHING in the group (matches Rose bot's 'owner' tier), unlike a
    regular promoted admin who is limited to managing only who THEY
    promoted."""
    if user_id == OWNER_ID:
        return True
    if has_group_access(chat_id, user_id):
        return True
    member = await get_admin_member(bot, chat_id, user_id)
...
def tokenize_custom_emojis(text, entities, added_by):
    """Converts every custom-emoji glyph in `text` into a {name} token tied
    to its exact custom_emoji_id (saving it first if it's new). Used
    wherever the admin is pinning ONE SPECIFIC premium emoji on purpose
    (e.g. setting a filter's reply) -- a {name} token always resolves back
    to that exact emoji, whereas a bare glyph character left in the text
    would go through the random-pick-among-alternates path in
    render_with_emojis() and could come back as a DIFFERENT saved emoji
    that happens to share the same fallback glyph. Text with no custom
    emoji in it is returned unchanged."""
    custom_ents = [e for e in (entities or []) if e.type == MessageEntity.CUSTOM_EMOJI]
    if not custom_ents:
        return text
    auto_save_new_emojis_from_message(text, entities, added_by)  # make sure every one of them has a saved name
    idx_map = _utf16_to_str_index_map(text)
    out = text
    for e in sorted(custom_ents, key=lambda ent: ent.offset, reverse=True):
        str_start = idx_map[e.offset]
        str_end = idx_map[e.offset + e.length]
        row = get_custom_emoji_by_id(e.custom_emoji_id)
        if not row:
            continue  # shouldn't happen right after auto-save above, but don't crash if it does
        out = out[:str_start] + "{" + row["name"] + "}" + out[str_end:]
    return out
...
# =============================================================================
# OPENROUTER (DeepSeek v3.1) AI LAYER -- DB-backed multi-key rotation + fallback
# =============================================================================
def add_ai_api_key(key, added_by):
    conn = db_connect()
    try:
        conn.execute(
            "INSERT OR IGNORE INTO ai_api_keys (api_key, added_by, added_at, disabled) VALUES (?,?,?,0)",
            (key, added_by, datetime.now(IST).isoformat()),
        )
        conn.commit()
        return conn.execute("SELECT COUNT(*) c FROM ai_api_keys WHERE disabled=0").fetchone()["c"]
    finally:
        conn.close()


def get_active_ai_api_keys():
    conn = db_connect()
    try:
        rows = conn.execute("SELECT id, api_key FROM ai_api_keys WHERE disabled=0 ORDER BY id").fetchall()
        return [(r["id"], r["api_key"]) for r in rows]
    finally:
        conn.close()


def disable_ai_api_key(key_id):
    conn = db_connect()
    try:
        conn.execute("UPDATE ai_api_keys SET disabled=1 WHERE id=?", (key_id,))
        conn.commit()
    finally:
        conn.close()


_ai_key_cursor = 0  # index into get_active_ai_api_keys() -- sticks here until this key fails


def _ai_call_raw(messages, json_mode=True, temperature=0.2, max_tokens=400, thinking=False):
    """Calls OpenRouter (DeepSeek v3.1). Uses whichever key is at
    _ai_key_cursor and STAYS on it across calls -- only moves to the next
    key when the current one actually fails (never round-robins per call).
    A clear auth failure (401/403 -- key is bad/revoked) disables that key
    in the DB permanently; a transient failure (429/5xx/timeout) just moves
    the cursor on for this run without disabling it.

    thinking=False (default): reasoning/thinking mode OFF -- fast, short
    answer, used for anything speed-sensitive (the ".." command path, the
    roast). thinking=True: reasoning ON -- slower but more thorough, used
    ONLY for "##" (self-aware chat mode), where a more thought-through
    answer is worth the extra second or two."""
    global _ai_key_cursor
    keys = get_active_ai_api_keys()
    if not keys:
        logger.error("No AI API keys configured! Owner needs to DM the bot /apisetup.")
        return None

    n = len(keys)
    last_error = None
    for attempt in range(n):
        idx = (_ai_key_cursor + attempt) % n
        key_id, key_value = keys[idx]
        try:
            payload = {
                "model": AI_MODEL,
                "messages": messages,
                ...
                # DeepSeek v3.1 is a "hybrid reasoning" model -- on some
                # routes it defaults to thinking-mode ON, which burns many
                # hidden reasoning tokens before the real answer. Off by
                # default for speed; only "##" opts into it (see docstring).
            ...
            resp = requests.post(
                ...
                json=payload,
                timeout=10,  # fail fast onto the next key instead of hanging
            )
            if resp.status_code == 200:
                _ai_key_cursor = idx  # stick to this key next time too
                content = resp.json()["choices"][0]["message"]["content"]
                return content
            elif resp.status_code in (401, 403):
                logger.warning("AI key #%s is invalid/revoked -- disabling it permanently.", key_id)
                disable_ai_api_key(key_id)
                last_error = f"HTTP {resp.status_code} (key disabled)"
                continue
            elif resp.status_code in (429, 500, 502, 503):
                last_error = f"HTTP {resp.status_code} on key #{attempt+1}"
                logger.warning("AI key temporarily failed (%s), moving to next key...", last_error)
                continue
            else:
            ...
            last_error = str(e)
            logger.warning("AI request error, moving to next key: %s", e)
            continue

    _ai_key_cursor = 0  # every key failed this round -- start from the top next time
    logger.error("All AI API keys failed this round. Last error: %s", last_error)
    return None
...
async def ai_classify_admin_command(command_text: str, has_reply_target: bool, recent_context: str = "") -> dict:
    """
    Ask the AI to understand a natural-language admin command (Hindi/English/
    Hinglish, any phrasing, any tone, EVEN WITH SPELLING MISTAKES) and turn
    it into one or more structured actions (a single message can contain
    multiple commands at once -- all get executed).
    Returns a dict like {"actions": [{"action": "mute", "duration_minutes": 60, "confidence": "high"}, ...]}
    """
    system_prompt = (
        ...
        "sarcastic, indirect, ghumaake bola hua, gaali-galoch/slang me, seedha "
        "command jaisa, YA SPELLING MISTAKES ke saath bhi ho sakta hai -- jaise "
        "'admin banaon' (matlab 'banao'), 'mut kardo' (matlab 'mute kardo'). Typos ko "
        "context se samajhkar sahi meaning nikaalo, kabhi bhi sirf spelling exact match "
        "na hone ki wajah se action miss mat karo). Tumhara kaam hai: poore sentence ka "
        "ASLI matlab aur intent samajhna, jaise ek samajhdaar insaan group ka context "
        ...
        "ISKO ZAROOR USE KARO:\n"
        "1. Agar current command apne aap me ambiguous lage (jaise sirf 'isko bhi' ya "
        "'isko theek karo') -- upar ki baatcheet dekhkar samjho kis cheez ka zikar ho raha hai.\n"
        "2. Agar admin kahe 'isko dhamki do' / 'isko sabak sikhao' / 'iski waat lagao' "
        "jaisa vague-severity wala command de (bina yeh bataye ki EXACTLY kya karna hai) "
        "-- to context padhkar khud decide karo ki severity kitni honi chahiye: agar "
        "target ne halki-si galti ki hai (thoda bura bola, spam kiya) to 'warn' choose "
        "karo; agar target ne repeatedly bura bartaav kiya hai, gaali-galoch/harassment "
        "kiya hai, ya baar-baar warning ke baad bhi nahi ruka -- to 'mute' choose karo. "
        "Apni reasoning field me yeh explain karo ki context me kya dekha aur kyun woh "
        "severity choose ki.\n\n"
        "Possible actions (in 'action' field, per item): "
        ...
        "dispute_unfreeze, escrow_verify, escrow_unverify, info, set_title, "
        "toggle_permission, grant_permission, unknown.\n\n"
        ...
        "Agar koi duration nahi bola gaya, mute PERMANENT hota hai (jab tak manually "
        "unmute na ho) -- 60 minute jaisa koi default MAT lagao, duration_minutes null "
        "hi rakho jab tak explicitly na bola jaye.\n"
        ...
        "Rules:\n"
        "- MULTIPLE COMMANDS: agar ek hi message me do ya zyada alag-alag commands "
        "diye gaye hain (jaise 'isko warn karo aur mute bhi kar do', 'isko kick karo, "
        "phir escrow verify bhi karo, aur naam Manager rakh do'), to 'actions' array "
        "me har ek ko ALAG object ke roop me do -- sab EXECUTE honge, chahe 2 ho, 3 "
        "ho, ya 4. Ek hi command ho to bhi array me EK hi object rakho.\n"
        ...
        "'yahan tak sab clean kar do', 'yahan tak delete karo', 'is se yahan tak hata "
        "do') = 'purge' -- CHAHE isme bhi 'delete' word hi kyun na use hua ho. "
        "'yahan tak' / 'is se lekar yahan tak' / 'up to here' jaisa koi bhi range-wala "
        "phrasing dikhe to HAMESHA 'purge' do, 'delete' word dekhkar confuse mat ho.\n"
        "- 'set_title': agar admin ne bracket () ke andar naya title diya hai (jaise "
        "'iska tag change karo (Manager)'), to 'title' field me bracket ke andar ka "
        "text hi do (backend anyway raw message se bracket content khud nikaal lega, "
        "yeh field sirf 'set_title' action detect karne ke liye hai).\n"
        "- 'toggle_permission': koi bhi group-wide setting on/off karne ka intent, "
        "jaise 'photo bhejna band karo', 'sirf photo band karo baaki sab chalne do', "
        "'videos allow kar do', 'polls band karo', 'link preview off karo', 'invite "
        "band karo'. 'permission' field me EK naam do: messages, photos, videos, "
        "audios, documents, stickers_gifs, polls, voice_notes, video_notes, "
        "link_previews, invite_users, pin_messages, change_info. 'enabled' field me "
        "true (allow/on karna hai) ya false (band/off karna hai) do. Sirf USS EK "
        "cheez ko badalna hai, baaki sab settings waisi hi rahein -- backend khud "
        "yeh merge karke handle karega.\n"
        "- 'grant_permission': ek REPLY ya @username se target diya gaya admin ko "
        "EXTRA admin-permissions dene ka intent, jaise 'isko 2,5 permission do', "
        "'iska ban karne ka permission on karo'. Yeh sirf ek ALREADY-admin user ke "
        "liye hota hai. Numbers ki list 'permission_numbers' field me do (list of "
        "int): 1=Delete messages, 2=Restrict/ban/mute members, 3=Pin messages, "
        "4=Invite users via link, 5=Manage video chats, 6=Change group info, "
        "7=Add new admins (promote others). Agar number na bola ho par naam bola ho "
        "(jaise 'ban karne ka permission do') to us naam ka number khud match karke "
        "do.\n"
        "- Agar duration bola gaya hai (jaise '10 minute', '1 ghanta', '2 din') to use "
        "minutes me convert karke us action ke 'duration_minutes' me do, warna null.\n"
        ...
        "casual wording ho, ya spelling galat ho) to closest action do, confidence "
        "'medium'/'low' de sakte ho par 'unknown' mat do.\n"
        f"- has_reply_target = {has_reply_target}.\n\n"
        "Top-level 'reason' field: agar admin ne KOI justification/wajah di hai command "
        "ke saath (jaise 'bahut gaali de raha hai isliye ban karo', 'spam kar raha hai') "
        "to woh chhota sa reason EXTRACT karke 'reason' field me do (English ya Hindi, "
        "jaisa admin ne bola). Agar koi wajah bilkul nahi di hai to 'reason': "
        "'Admin request' do.\n\n"
        "Pehle top-level 'reasoning' field me 2-3 chhoti lines me socho ki command(s) "
        "ka asli matlab kya hai (context ko dhyan me rakhkar, typos ko bhi resolve "
        "karke), FIR 'actions' array decide karo -- yeh reasoning step tumhari accuracy "
        "zyada better banata hai, isliye skip mat karo. Ek message me 2, 3, 4, ya "
        "zyada alag commands ho sakte hain -- SAB ko 'actions' array me include karo, "
        "koi bhi miss mat karo.\n\n"
        'Respond ONLY with compact JSON: {"reasoning": "...", "reason": "Admin request", "actions": '
        '[{"action": "...", "duration_minutes": null, "title": null, "count": 1, '
        '"permission": null, "enabled": null, "permission_numbers": null, "confidence": "high|medium|low"}]}'
    )
    ...
    if raw is None:
        return {"actions": [{"action": "unknown", "duration_minutes": None, "confidence": "low", "error": "ai_unavailable"}]}
    try:
        parsed = json.loads(raw)
        if "actions" not in parsed and "action" in parsed:
            parsed = {"actions": [parsed]}
        return parsed
    except Exception:
        logger.warning("Could not parse AI response: %s", raw)
        return {"actions": [{"action": "unknown", "duration_minutes": None, "confidence": "low"}]}

_MANAGER_ROASTS = [
    "😂 <b>{name} khud ko CEO samajhta hai, hakikat mein unpaid intern hai.</b>",
    "🤡 <b>{name}, himmat dekho — zero power, full boss attitude.</b>",
    "🙄 <b>{name}, tu manager nahi, is group ka background extra hai.</b>",
    "😹 <b>{name} khud ko boss samajh raha, sab peeche se hass rahe hain.</b>",
    "💀 <b>{name}, admin panel dekha bhi nahi, order kaise de raha?</b>",
    "😏 <b>{name} mein itni entitlement kahan se, resume mein 'admin' likha kya?</b>",
    "🤣 <b>{name}, naukar hai kya tu uska? Member hai, haq mat jata.</b>",
    "😆 <b>{name}, tu group mein utna hi important jitna 'last seen recently'.</b>",
    "🙃 <b>{name} ki permission list khaali, ego full tank hai.</b>",
    "🫠 <b>{name}, teri authority ka level bilkul 'guest' jaisa hai.</b>",
    "😹 <b>{name} ne dimaag mein khud ko promote kar liya, record mein member hi hai.</b>",
]

_last_manager_roast_fallback = None  # index of the last fallback line used -- never repeat it back-to-back


async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    """A regular (non-admin) member called for 'manager' -- fresh AI-generated
    savage Hinglish roast every single time, personal to whatever they
    actually typed. The AI is told to silently think up a few options and
    pick its single sharpest one -- only that ONE final roast line gets
    sent, no visible ranking/scoring/multiple-options shown in chat. Falls
    back to the static pool ONLY if the AI call itself fails (no API key /
    network / rate limit) -- the fallback explicitly excludes whichever
    line it used last time, so even several failed-AI-calls in a row can
    never repeat back-to-back."""
    system_prompt = (
        "You are Roast King -- India's most savage roaster running inside this "
        "Telegram group's manager bot. A NORMAL member (not an admin) just "
        "called/asked for the 'manager' as if they run the place -- roast them "
        "for it, using their first name.\n\n"
        "PROCESS (do this silently, don't show your work): think up 2-3 "
        "different savage roast angles for this person, judge which one hits "
        "hardest, then output ONLY that single winning line. Never show the "
        "other options, never show a ranking, score, or explanation -- just "
        "the one final roast, nothing else in your response.\n\n"
        "RULES for that one line:\n"
        "1. ONE short punchy WhatsApp-style line, 5-14 words max. No long "
        "sentences, no second line.\n"
        "2. Language: 100% Roman Hinglish, exactly like people chat on "
        "WhatsApp -- NEVER Devanagari script. Keep vocabulary simple and "
        "everyday, not overly literary and not incoherent slang either -- a "
        "normal person should instantly get the joke on first read.\n"
        "3. Must reference their name directly or through a clever pun/"
        "wordplay on it -- make it feel personal, not generic.\n"
        "4. Build it around ONE clever, concrete, unexpected comparison "
        "(apps, everyday digital-life stuff, common habits) -- show the "
        "joke, don't just state 'you have ego'.\n"
        "5. Tone: confident, sarcastic, cutting, zero apology or softening. "
        "But NO slurs, no swear words/profanity, no casteist/religious/"
        "communal attacks, no real threats, nothing about body/family/"
        "appearance/protected traits -- the burn comes purely from wit and a "
        "sharp comparison, never vulgarity.\n"
        "6. Max 1 emoji.\n"
        "7. Every single time, use a genuinely different angle from before -- "
        "never repeat a comparison.\n"
        "8. No apology, no disclaimer, no 'just kidding' -- full attitude, "
        "but always something a normal person finds funny, not confusing."
    )
    user_content = f"User '{first_name}' just sent this message: \"{message_text}\""
    raw = await asyncio.to_thread(
        _ai_call_raw,
        [
            {"role": "system", "content": system_prompt},
    ...
    if not raw:
        logger.warning("ai_roast_manager_caller: AI call returned nothing, using fallback pool.")
        global _last_manager_roast_fallback
        choices = list(range(len(_MANAGER_ROASTS)))
        if _last_manager_roast_fallback in choices and len(choices) > 1:
            choices.remove(_last_manager_roast_fallback)
        idx = random.choice(choices)
        _last_manager_roast_fallback = idx
        return _MANAGER_ROASTS[idx].format(name=md_escape(first_name))
    return raw.strip()
...
def get_feature_docs() -> str:
    """Built as a function (not a plain string) so it can pull the ACTUAL
    live constants (WARN_BAN_LIMIT etc, defined later in the file) -- this
    is what the '##' self-aware mode reads from, so it must always reflect
    real current values, never stale/guessed numbers."""
    return f"""
...
ko REPLY karke, EK saath MULTIPLE commands bhi de sakte hain ek message me):
ban, unban, kick, mute (duration ke saath -- koi duration na bole to PERMANENT
mute hota hai jab tak manually unmute na ho), unmute, warn (count ke saath,
jaise "2 warning do"; default {WARN_BAN_LIMIT} warnings pe action lagta hai,
lekin har group /setwarnlimit aur /setwarnaction se apna limit/action (ban/
kick/mute) customize kar sakta hai), unwarn (count ke saath, ya "saari hata do"), promote, demote, pin, unpin,
delete, info (user ki jaankari), escrow_verify, escrow_unverify, set_title
(admin ka custom tag/title badalna -- bracket () ke andar likhna zaroori hai,
jaise "iska tag change karo (Manager)"), toggle_permission (group-wide
settings jaise "photo band karo baaki allow rakho", "polls off karo",
"invite band karo" -- sirf usi ek cheez ko badalta hai, baaki settings waisi
hi rehti hain), grant_permission (kisi already-admin ko extra rights dena).

PROMOTE DEFAULTS: jab bhi kisi ko admin banaya jaata hai, use SIRF yeh 4
cheezein milti hain by default: Delete messages, Pin messages, Invite users
via link, aur Warn karna (warn koi Telegram-right nahi hai, bot ka apna
feature hai isliye har admin ko hamesha milta hai). Mute/ban/kick karne ka
right NAHI milta by default. Extra rights dene ke liye, us admin ko reply "
karke ya @username leke number bolo (jaise "isko 2,5 permission do"):
1=Delete messages, 2=Restrict/ban/mute members, 3=Pin messages,
4=Invite users via link, 5=Manage video chats, 6=Change group info,
7=Add new admins (promote others). Sirf group ka owner, ya jisne us admin
ko khud promote kiya tha, extra permissions de sakta hai ya use demote kar
sakta hai -- koi aur admin nahi.

PER-ADMIN REAL PERMISSION CHECK: bot khud har admin ka ASLI Telegram-level
permission check karta hai har command se pehle -- agar us admin ke paas
woh specific right (jaise restrict members, promote members, pin messages,
delete messages, ya change info) nahi hai, to bot use clearly bata deta hai
ki "bot ke paas yeh permission hai, par TUMHARE paas nahi hai", clickable
mention ke saath -- kabhi vague ya galat reason nahi deta. Sirf bot-owner
ke liye yeh check skip hota hai (owner sab kuch kar sakta hai).

OWNER-ONLY commands (sirf bot owner, ".." ke saath): purge (reply se lekar
latest tak sab delete), lock (chat lock), unlock, upgrade_trust/
downgrade_trust (Trusted Trader level -- count ke saath, jaise "2 level upgrade
karo"; "max/full" bolne se seedha top level pe), dispute_freeze/
dispute_unfreeze (jhagde me dono users ko temporarily mute).

DIRECT (non-AI) COMMANDS -- deterministic, no ".." needed:
- Kisi ko reply karke sirf "info" likhna -- turant uski jaankari deta hai.
- "group id" ya "chat id" likhna (admin) -- turant is group ki chat ID deta hai.
- "filter karo (trigger)" (kisi message ko reply karke) -- Rose-bot jaisa
  keyword/command auto-reply set karta hai. "filter hata do (trigger)" se
  remove. /filters se list.
- /rules aur /setrules <text> (admin) -- group rules set/dikhana.
- /welcome on ya /welcome off (admin) -- is group me welcome message
  on/off karna.
- /welcomeimage on ya /welcomeimage off (admin) -- naye member ki apni
  profile photo welcome message ke saath dikhegi ya nahi (custom welcome
  image/video set ho to woh hamesha dikhega, isse independent).

TRUST LEVELS: New Member -> Known Member -> Trusted Trader -> Verified
Trader (total {TRUST_LEVEL_MAX + 1} levels, 0 se {TRUST_LEVEL_MAX} tak). Sirf
owner upgrade/downgrade kar sakta hai. /trusted command se group me sabhi
Trusted Traders ki list dikhti hai.

WELCOME MESSAGE CUSTOMIZATION (owner /start panel se, ya granted group-admin
/mygroup <chat_id> se): welcome text bilkul EXACT font/style me set hota hai
jaisa bheja jaaya (auto-styling nahi hota agar custom text set hai). Available
placeholders: {{name}} {{group}} {{id}} {{username}} {{profile}} -- inhe
welcome text me kahin bhi likh sakte hain, actual value se replace ho jaate
hain. Image/video bhi caption ke saath bhej sakte hain -- woh member ki apni
profile photo ki jagah fix media ke roop me set ho jaata hai. Button
text/link aur channel link bhi customize kar sakte hain.

CUSTOM/PREMIUM EMOJI: Owner DM me seedha koi bhi premium emoji bhej de --
automatic save ho jaata hai (koi command nahi chahiye). Uske baad {{label}}
placeholder se ya seedha wahi emoji type karke kahin bhi (welcome message,
filters, sab jagah) use ho jaata hai -- aur jahan bhi uska NORMAL/plain
alternate emoji bot ke kisi bhi message me aata hai, wahan bhi automatic
premium wala use hota hai. /emojis, /renameemoji, /delemoji se manage karo.

MULTI-GROUP ACCESS: Owner /grant <user_id> <group_id> se kisi ko us EK group
ke liye access de sakta hai -- woh us group ka "Owner" ban jaata hai (info
me Owner/max-trust/escrow-verified dikhta hai), aur /mygroup <group_id> se
apna welcome/button/channel customize kar sakta hai. /revoke <group_id> se
group ki approval hi hata sakte ho (bot wahan kuch nahi karega), /revoke
<user_id> <group_id> se sirf uss ek user ka access.

GROUP APPROVAL: Bot kisi bhi group me tab tak kaam nahi karta jab tak owner
/approve <group_id> (ya /grant) se approve na kare. Bina approval ke group me
bot chup rehta hai, sirf ek baar bolta hai ki owner se approval lo.

AUTOMATIC PROTECTION (kuch bolna nahi padta, khud chalta hai):
- Naye member ko photo/video + welcome message + escrow disclaimer + channel
  buttons.
- Gaali dene par: message delete + mute (default {ABUSE_MUTE_MIN} min, per-group
  customizable via /setabusemute) + warning (on/off via /setabusewarn).
- Link/username/promotion bhejne par: message delete + warning, instant
  local detection (username detection hata diya gaya hai, sirf real
  links/domains pakde jaate hain). Mute yahan by default OFF hai, per-group
  on karne ke liye /setlinkmute.
- Deal/buy-sell keywords (jaise "sell", "buy", "escrow", "kharidna") pe
  automatic trusted escrowers ki list aati hai.
- Escrow impersonation guard: koi khud ko escrow provider bataye (owner ke
  alawa) to turant block, aur escrow-verified users ki list + owner ka
  button dikhta hai.
- Antiflood: {ANTIFLOOD_MAX_MSGS}+ messages {ANTIFLOOD_MUTE_MIN} min ke andar
  bhejne par mute.
...
- Anti-raid: {RAID_JOIN_THRESHOLD}+ members ek saath ({RAID_JOIN_WINDOW}
  second ke andar) join karein to uske baad aane wale sab members
  auto-remove hote hain jab tak raid rukta nahi.
- Admin/Owner impersonation detector: naya member agar naam/username/photo
  me owner se match kare to warning aati hai.
...
MODERATION SETTINGS COMMANDS (kisi bhi group-admin ke liye, group me seedha
CHALAO, YA bot ko DM karke -- DM me command ke turant baad group_id bhi dena
padta hai kyunki DM me chat implicit nahi hoti):
- /setabusemute <minutes|off> -- gaali par kitni der mute (0/off = mute nahi).
- /setlinkmute <minutes|off> -- link/promotion par kitni der mute.
- /setabusewarn <on|off> -- gaali par warning milegi ya nahi.
- /setlinkwarn <on|off> -- link/promotion par warning milegi ya nahi.
- /setwarnlimit <number> -- kitni warnings pe action (ban/kick/mute) lagta hai.
- /setwarnaction <ban|kick|mute> -- warn-limit paar karne par kya hota hai.
- /modsettings -- is group ki saari upar wali settings ek saath dikhata hai.
DM se use karna ho to: /setabusemute <group_id> <minutes|off>, isi tarah baaki
sab commands me bhi group_id command ke turant baad, value uske baad.

ADMIN PANEL: Bot ko DM me /start bhejo (sirf owner). Wahan se "My Groups"
(welcome/button per group), "Save Custom Emoji", welcome channels manage,
database export/import, aur trusted traders dekh sakte ho.

CHAT MODE: Message ke aakhir me ".." lagane se admin-command mode chalta
hai. "##" lagane se yeh self-aware chat mode chalta hai (SIRF BOT OWNER ke
liye, granted group-owners ke liye NAHI) -- bot ke features ke baare me
sawal poochne ke liye, ya kisi user ke message ko reply karke uske baare
me specific sawal poochne ke liye (jaise "iska kitna warn hua hai").
"""


async def ai_owner_chat(question: str, user_data_context: str = "", chat_id: int = None) -> str:
    ...
    if chat_id is not None:
        limit, action = await asyncio.to_thread(get_warn_settings, chat_id)
        abuse_mute, abuse_warn, link_mute, link_warn = await asyncio.to_thread(get_abuse_link_settings, chat_id)
        quick_facts = (
            "THIS GROUP's CURRENT LIVE SETTINGS -- these are the REAL, exact "
            "numbers for THIS group right now. If the question is about any "
            "limit/duration/on-off setting, the answer is HERE -- use it "
            "exactly, never say 'no fixed limit' or 'pata nahi' for anything "
            "listed below:\n"
            f"- Warn limit: {limit} warnings -> action: {action}\n"
            f"- Abuse (gaali) -> mute: {(str(abuse_mute) + ' min') if abuse_mute else 'OFF'}, "
            f"warn: {'ON' if abuse_warn else 'OFF'}\n"
            f"- Link/promotion -> mute: {(str(link_mute) + ' min') if link_mute else 'OFF'}, "
            f"warn: {'ON' if link_warn else 'OFF'}\n"
            f"- Antiflood: {ANTIFLOOD_MAX_MSGS}+ messages in {ANTIFLOOD_WINDOW}s -> {ANTIFLOOD_MUTE_MIN} min mute\n"
            f"- Anti-raid threshold: {RAID_JOIN_THRESHOLD}+ joins in {RAID_JOIN_WINDOW}s\n\n"
        )
    system_prompt = (
        ...
        + "BAHUT ZAROORI RULE 1: Owner ne jo bhi poocha hai, SIRF usi ka jawab do -- "
        ...
        "BAHUT ZAROORI RULE 2: Poori documentation (quick-facts + neeche wali dono) "
        "dhyan se pehle poori padho, PHIR jawab do. Kisi bhi number/limit/setting ke "
        "baare me 'iska koi fix limit nahi hai' ya 'pata nahi' TABHI bolo jab woh "
        "cheez genuinely doc mein kahin bhi mention hi na ho -- agar woh number "
        "doc mein kahin bhi likha hai (chahe kisi lambe sentence ke beech mein hi "
        "kyun na ho), wahi exact number confidently do.\n"
    ...
    raw = await asyncio.to_thread(
        ...
        temperature=0.4,
        max_tokens=900,
        thinking=True,  # "##" is the one place a slower, more thought-through answer is worth it
    )
    if raw is None:
...
DEAL_KEYWORDS_RE = re.compile(
    ...
    r"deal|dealing|dealer|trade|trading|trader|exchange|swap|"
    r"escrow|middleman|mediator|guarantor|"
    r"kharid\w*|bech\w*|bik\w*|bikri|len\s*den|lena\s*dena|"
    r"cod|advance\s*payment|payment\s*first|pehle\s*payment|paisa\s*pehle|"
    r"vendor|purchase|price\s*fix|rate\s*fix|final\s*price|"
    r"id\s*sell|id\s*buy|account\s*sell|account\s*buy|acc\s*sell|acc\s*buy|"
    r"top\s*up|topup|recharge|diamonds?\s*sell|diamonds?\s*buy|"
    r"payment\s*proof|screenshot\s*bhejo|paisa\s*bhejo|paisa\s*de\s*do|"
    r"transaction|upi|paytm|gpay|phonepe|bank\s*transfer|"
    r"interested\?|dm\s*me|dm\s*karo|price\s*batao|rate\s*batao|kitne\s*ka"
    r")\b",
    re.IGNORECASE,
)
...
_admin_member_cache = {}  # (chat_id,user_id) -> (ChatMember, ts)
_pending_permission_grant = {}  # (chat_id, admin_id) -> (target_id, timestamp)


async def get_admin_member(bot, chat_id, user_id):
    """Cached ChatMember object (has all the real can_* rights) for a
    specific admin -- used to check what THEY are actually allowed to do,
    not just whether they're an admin at all."""
    key = (chat_id, user_id)
    ...
    try:
        ...
        member = None
    _admin_member_cache[key] = (member, now)
    return member


# Which real Telegram admin-right is needed for each bot action.
ACTION_PERMISSION_REQUIREMENT = {
    "ban": "can_restrict_members", "unban": "can_restrict_members",
    "kick": "can_restrict_members", "mute": "can_restrict_members",
    "unmute": "can_restrict_members",
    "promote": "can_promote_members", "demote": "can_promote_members",
    "set_title": "can_promote_members",
    "pin": "can_pin_messages", "unpin": "can_pin_messages",
    "delete": "can_delete_messages", "purge": "can_delete_messages",
    "toggle_permission": "can_restrict_members",
    "lock": "can_change_info", "unlock": "can_change_info",
}
PERMISSION_LABELS = {
    "can_restrict_members": "members ko restrict karne (ban/mute/kick/warn)",
    "can_promote_members": "admins manage karne (promote/demote/tag change)",
    "can_pin_messages": "message pin/unpin karne",
    "can_delete_messages": "message delete karne",
    "can_change_info": "group settings badalne",
}


async def check_permission_or_explain(context, chat, admin_user, action) -> bool:
    """Checks whether admin_user ACTUALLY has the Telegram-level right
    needed for this action (not just 'is an admin'). If missing, replies
    with a clear, professional, 100%-accurate reason (never a vague
    generic error) with a clickable mention -- so it's obvious the bot
    itself is fine, it's specifically THIS admin's permission that's short."""
    if admin_user.id == OWNER_ID:
        return True
    needed = ACTION_PERMISSION_REQUIREMENT.get(action)
    if not needed:
        return True
    member = await get_admin_member(context.bot, chat.id, admin_user.id)
    if member is None:
        return True  # couldn't verify -- fail open rather than block wrongly
    if member.status == ChatMemberStatus.OWNER:
        return True
    has_it = bool(getattr(member, needed, False))
    if has_it:
        return True
    try:
        await context.bot.send_message(
            chat.id,
            stylize(f"⚠️ <a href='tg://user?id={admin_user.id}'>{md_escape(admin_user.first_name)}</a>, "
            f"<b>bot theek se kaam kar raha hai, lekin aapke paas is group me "
            f"{PERMISSION_LABELS.get(needed, needed)} ki admin-permission nahi hai.</b> "
            f"Yeh Telegram-level permission hai -- group ke asli owner se apna admin "
            f"rights badhwao, tabhi yeh command chalega."),
...
# =============================================================================
_channel_title_cache = {}   # username -> (title, fetched_at)


async def _channel_display_title(context, username: str) -> str:
    cached = _channel_title_cache.get(username)
    ...
    try:
        ...
        pass
    _channel_title_cache[username] = (title, time.time())
    return title
...
async def send_welcome_message(context: ContextTypes.DEFAULT_TYPE, chat, member):
    ...
    if gsettings.get("welcome_enabled") == 0:
        return  # /welcome off -- do nothing at all
    ...
    has_custom_text = bool(gsettings.get("welcome_text"))
    ...
    try:
        caption = template.format(
            ...
            profile=f"<a href='{profile_link}'>{name}</a>",
        )
    ...
    if gsettings.get("channel_username"):
        ch_label = await _channel_display_title(context, gsettings["channel_username"])
        buttons.append([InlineKeyboardButton(ch_label, url=f"https://t.me/{gsettings['channel_username']}", style="success", icon_custom_emoji_id=CHANNEL_BUTTON_EMOJI_ID)])
    else:
        for ch_username in get_welcome_channels():
            ch_label = await _channel_display_title(context, ch_username)
            buttons.append([InlineKeyboardButton(ch_label, url=f"https://t.me/{ch_username}", style="success", icon_custom_emoji_id=CHANNEL_BUTTON_EMOJI_ID)])
    ...
    if gsettings.get("button_text") and gsettings.get("button_url"):
        ...
        owner_label, owner_url = await asyncio.to_thread(get_group_owner_contact, chat.id)
        buttons.append([InlineKeyboardButton(fancy(owner_label), url=owner_url, style="primary", icon_custom_emoji_id=CONTACT_OWNER_BUTTON_EMOJI_ID)])
    if gsettings.get("rules_text"):
        buttons.append([InlineKeyboardButton(
            fancy("Rules"), callback_data="show_rules", style=BTN_GREEN, icon_custom_emoji_id=RULES_BUTTON_EMOJI_ID,
        )])
    markup = InlineKeyboardMarkup(buttons)

    # Custom welcome media (set via the panel) always wins -- many group
    # owners don't want the NEW MEMBER's own profile photo shown at all,
    # they want their own fixed banner/logo image or video instead.
    custom_media_id = gsettings.get("welcome_media_file_id")
    custom_media_type = gsettings.get("welcome_media_type")

    photo_bytes = None
    if not custom_media_id and gsettings.get("welcome_profile_pic_enabled", 1) != 0:
        # Profile picture ALWAYS attaches (if the member has one) regardless
        # of any welcome-text customization -- unless a custom media is set,
        # or the group has turned this off via /welcomeimage off.
        try:
    ...
    final_text, final_entities = render_with_emojis(caption, fancy_enabled=not has_custom_text)
    try:
        if custom_media_id and custom_media_type == "video":
            await context.bot.send_video(chat.id, video=custom_media_id, caption=final_text, caption_entities=final_entities, reply_markup=markup)
        elif custom_media_id and custom_media_type == "animation":
            await context.bot.send_animation(chat.id, animation=custom_media_id, caption=final_text, caption_entities=final_entities, reply_markup=markup)
        elif custom_media_id:  # photo
            await context.bot.send_photo(chat.id, photo=custom_media_id, caption=final_text, caption_entities=final_entities, reply_markup=markup)
        elif photo_bytes:
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------------------------------------------------------
    # DETERMINISTIC "admin list" QUERY -- live from Telegram, no DB/AI
    # ---------------------------------------------------------------
    _admlist_probe = (msg.text or msg.caption or "").strip().lower()
    if re.search(r"\badmin\s*(list|s)?\b.*\b(dikhao|batao|show|list)\b|\bshow\s*admins\b|\badmin\s*list\b", _admlist_probe) and len(_admlist_probe.split()) <= 8:
        await admins_list_command(update, context)
        return
    ...
    # Passively learn any premium/custom emoji used in ANY message in a
    # group the bot is in -- not just the explicit "filter (...)" flow.
    # Silent, self-deduping (auto_save_new_emojis_from_message skips ones
    # already saved), builds up the group's emoji + name/alternative table
    # automatically over time.
    _emoji_entities = msg.entities if msg.text else msg.caption_entities
    if _emoji_entities:
        await asyncio.to_thread(auto_save_new_emojis_from_message, text, _emoji_entities, user.id)
    ...
    # ---------------------------------------------------------------
    # Loosened on purpose: the old version required the exact verb
    # ("kar.../set kar.../hata...") right before "filter" AND the
    # "(trigger)" to be the very last thing in the message. Any other
    # phrasing (extra words, a different verb, trailing text after the
    # parentheses) silently missed this match and fell through to the
    # general AI command classifier instead -- which isn't built for
    # filters and would sometimes guess "mute"/"kick" against whoever
    # sent it, since they were replying to their own message. Now it
    # just needs the word "filter" anywhere, followed eventually by a
    # "(...)" group anywhere in the same message.
    filter_match = re.search(r"\bfilter\w*\b[^(]*\(([^)]+)\)", text, re.IGNORECASE | re.DOTALL)
    if filter_match:
        # Filters are owner-tier only (bot owner, or whoever's been granted
        # owner access to this specific group) -- not every regular admin.
        is_really_admin = is_owner(user.id) or await asyncio.to_thread(has_group_access, chat.id, user.id)
        if not is_really_admin:
            return  # not owner-tier -- silently ignore, don't even acknowledge the attempt
        ...
        if trigger:
            if is_remove:
                ...
                if reply_text_val:
                    # Pin whichever SPECIFIC premium emoji the admin used here
                    # -- as a {name} token -- so it never gets swapped out for
                    # a different saved emoji later (see tokenize_custom_emojis).
                    reply_text_val = await asyncio.to_thread(
                        tokenize_custom_emojis, reply_text_val, rmsg.entities or rmsg.caption_entities, user.id,
                    )
    ...
    # ---------------------------------------------------------------
    # 0.71 BLACKLISTED WORDS -- delete + warn for non-admins
    # ---------------------------------------------------------------
    if not admin_ok:
        bad_word = await asyncio.to_thread(text_has_blacklisted_word, chat.id, text)
        if bad_word:
            try:
                await msg.delete()
            except TelegramError:
                pass
            count, actioned = await apply_warn_and_maybe_ban(context, chat.id, user.id, f"Blacklisted word: {bad_word}")
            try:
                await context.bot.send_message(
                    chat.id,
                    stylize(f"🚫 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, <b>blacklisted "
                    f"word use karne ke liye message delete kar diya gaya.</b> ⚠️ Warning ({count})"),
                    parse_mode=ParseMode.HTML,
                )
            except TelegramError:
                pass
    ...
    # ---------------------------------------------------------------
    # 0.71 "ID" TRIGGER -- exact whole message only ("id"/"ID"/"Id"), so it
    #      never fires on a word that merely CONTAINS "id" (e.g. "kid").
    #      Works for anyone, no admin check needed -- it's just info.
    # ---------------------------------------------------------------
    if text.strip().lower() == "id":
        try:
            await msg.reply_text(stylize(id_reply_text(chat, user)), parse_mode=ParseMode.HTML)
        except TelegramError:
            pass
        return

    # ---------------------------------------------------------------
    # 0.72 "MANAGER" KEYWORD TRIGGER -- admins get an instant obedient
    #      reply (no AI call, zero latency). Regular members calling for
    #      the manager get a fresh, unique AI-generated roast every time.
    # ---------------------------------------------------------------
    if re.search(r"\b(manager|मैनेजर)\b", text, re.IGNORECASE) and not text.strip().endswith("..") and not text.strip().endswith("##"):
        if admin_ok:
            final_text, final_entities = render_with_emojis("🫡 <b>Ji boss! Hukum kariye, kya kaam hai?</b>")
            try:
                await context.bot.send_message(chat.id, final_text, entities=final_entities, reply_to_message_id=msg.message_id)
            except TelegramError:
                pass
        else:
            roast = await ai_roast_manager_caller(text, short_name(user.first_name))
            final_text, final_entities = render_with_emojis(roast)
            try:
                await context.bot.send_message(chat.id, final_text, entities=final_entities, reply_to_message_id=msg.message_id)
            except TelegramError:
                pass
    ...
    # ---------------------------------------------------------------
    # 0.75 PENDING "WHICH EXTRA PERMISSIONS?" FOLLOW-UP -- after a promote,
    #      admin can reply with comma-separated numbers (e.g. "1,3,5") to
    #      grant those specific extra rights to the just-promoted user.
    # ---------------------------------------------------------------
    if admin_ok and re.fullmatch(r"[\d\s,]+", text.strip()):
        pending = _pending_permission_grant.get((chat.id, user.id))
        if pending:
            target_id, ts = pending
            if (now - ts) < 300:  # 5 minute window
                numbers = {int(n) for n in re.findall(r"\d+", text)}
                rights_map = {
                    1: "can_delete_messages", 2: "can_restrict_members", 3: "can_pin_messages",
                    4: "can_invite_users", 5: "can_manage_video_chats", 6: "can_change_info",
                    7: "can_promote_members",
                }
                grant_kwargs = {v: True for k, v in rights_map.items() if k in numbers}
                if grant_kwargs:
                    try:
                        target_member = await get_admin_member(context.bot, chat.id, target_id)
                        current_rights = {
                            f: getattr(target_member, f, False) for f in rights_map.values()
                        } if target_member else {}
                        current_rights.update(grant_kwargs)
                        current_rights["is_anonymous"] = False
                        await context.bot.promote_chat_member(chat.id, target_id, **current_rights)
                        granted_labels = ", ".join(rights_map[k].replace("can_", "").replace("_", " ").title() for k in sorted(numbers) if k in rights_map)
                        await msg.reply_text(
                            stylize(f"✅ <b>Extra permissions de diye:</b> {granted_labels}"),
                            parse_mode=ParseMode.HTML,
                        )
                    except TelegramError as e:
                        await msg.reply_text(stylize(f"❌ <b>Permissions dene me dikkat aayi:</b> <code>{md_escape(str(e))}</code>"), parse_mode=ParseMode.HTML)
                    del _pending_permission_grant[(chat.id, user.id)]
                    return
            else:
                del _pending_permission_grant[(chat.id, user.id)]
    ...
    # ---------------------------------------------------------------
    if DEAL_KEYWORDS_RE.search(text):
        last_notice = _deal_notice_last.get(chat.id, 0)
        if (now - last_notice) > DEAL_NOTICE_COOLDOWN_SECONDS:
            _deal_notice_last[chat.id] = now
            verified_rows = await asyncio.to_thread(get_escrow_verified_users, chat.id)
            user_lines = []
            for r in verified_rows:
                if r["username"]:
                    user_lines.append(f"• <a href='https://t.me/{r['username']}'>@{md_escape(r['username'])}</a>")
                else:
                    user_lines.append(f"• <a href='tg://user?id={r['user_id']}'>{md_escape(r['first_name'])}</a>")
            users_block = "\n".join(user_lines) if user_lines else "Koi escrow-verified user nahi hai abhi."
            owner_label, owner_url = await asyncio.to_thread(get_group_owner_contact, chat.id)
            markup = InlineKeyboardMarkup(
                [[InlineKeyboardButton(fancy(owner_label), url=owner_url, style="primary", icon_custom_emoji_id=CONTACT_OWNER_BUTTON_EMOJI_ID)]]
            )
            final_text, final_entities = render_with_emojis(
                f"💎 <b>Deal karni hai?</b> Sirf inse karo:\n{users_block}\n\n🚫 Kisi aur se ki to guarantee nahi."
            )
            try:
                await context.bot.send_message(chat.id, final_text, entities=final_entities, reply_markup=markup)
            except TelegramError:
                pass
    ...
    if not admin_ok and contains_abuse(text):
        ...
        if verdict.get("is_abuse"):
            try:
                ...
                pass
            abuse_mute_min, abuse_warn_enabled, _, _ = await asyncio.to_thread(get_abuse_link_settings, chat.id)
            if abuse_mute_min > 0:
                await mute_user(context, chat.id, user.id, abuse_mute_min)
            count, banned = (0, False)
            if abuse_warn_enabled:
            ...
            try:
                if banned:
                    await context.bot.send_message(
                        ...
                        f"warning-limit paar karne ke baad action liya gaya (ban/kick/mute -- group setting ke hisaab se).</b>"),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    bits = []
                    if abuse_mute_min > 0:
                        bits.append(f"{abuse_mute_min} min ke liye mute")
                    if abuse_warn_enabled:
                        bits.append(f"⚠️ Warning ({count})")
                    action_text = " + ".join(bits) if bits else "message delete"
                    await context.bot.send_message(
                        chat.id,
                        stylize(f"🤬 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a> <b>ne gaali di — "
                        f"{action_text} diya gaya.</b>"),
                        parse_mode=ParseMode.HTML,
                    )
            except TelegramError:
                pass
            ...
            if result.get("is_impersonation"):
                ...
                for u in verified_users:
                    if u["username"]:
                        ...
                        user_lines.append(f"• <a href='tg://user?id={u['user_id']}'>{md_escape(u['first_name'])}</a>")
                users_block = "\n".join(user_lines) if user_lines else "Abhi koi escrow-verified user nahi hai."
                owner_label, owner_url = await asyncio.to_thread(get_group_owner_contact, chat.id)
                markup = InlineKeyboardMarkup(
                    [[InlineKeyboardButton(fancy(owner_label), url=owner_url, style="primary", icon_custom_emoji_id=CONTACT_OWNER_BUTTON_EMOJI_ID)]]
                )
                final_text, final_entities = render_with_emojis(
                    f"⚠️ <b>Fake escrow alert!</b> Sirf inse contact karein:\n{users_block}\n\n👇 Ya niche wale se:"
                )
                await context.bot.send_message(
                    chat.id,
                    final_text,
    ...
    if not admin_ok and message_has_link_pattern(text):
        try:
            ...
            pass
        _, _, link_mute_min, link_warn_enabled = await asyncio.to_thread(get_abuse_link_settings, chat.id)
        if link_mute_min > 0:
            await mute_user(context, chat.id, user.id, link_mute_min)
        count, banned = (0, False)
        if link_warn_enabled:
        ...
        try:
            if banned:
                await context.bot.send_message(
                    chat.id,
                    stylize(f"🔨 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a> <b>ko "
                    f"warning-limit paar karne ke baad action liya gaya (group setting ke hisaab se).</b>"),
                    parse_mode=ParseMode.HTML,
                )
            else:
                bits = []
                if link_mute_min > 0:
                    bits.append(f"{link_mute_min} min ke liye mute")
                if link_warn_enabled:
                    bits.append(f"⚠️ Warning ({count})")
                action_text = " + ".join(bits) if bits else "message delete"
                await context.bot.send_message(
                    ...
                    f"warna ban kar diye jaoge!</b> {action_text}"),
                    parse_mode=ParseMode.HTML,
                )
        except TelegramError:
            pass
    ...
    if admin_ok and msg.reply_to_message is not None and "info" in _info_words and len(_info_words) <= 4:
        ...
        if t is not None and not t.is_bot:
            ...
            status_label, escrow = get_display_status(chat.id, t.id, trust, row["escrow_verified"] if row else 0)
            ...
            info_text = (
                ...
                f"<b>Trust Level:</b> {status_label}\n"
    ...
    if text.strip().endswith("##"):
        is_owner_tier = is_owner(user.id) or await asyncio.to_thread(has_group_access, chat.id, user.id)
        if not is_owner_tier:
            try:
                await context.bot.send_message(
                    chat.id,
        ...
        answer = await ai_owner_chat(question, user_data_context, chat_id=chat.id)
        try:
    ...
    if text.strip().endswith(".."):
        if not admin_ok:
            name_link = f"<a href='tg://user?id={user.id}'>{md_escape(short_name(user.first_name))}</a>"
            roast = random.choice(_NOT_ADMIN_ROASTS).format(name=name_link)
            final_text, final_entities = render_with_emojis(roast)
            try:
                markup = await owner_credit_button(context)
                await context.bot.send_message(
                    chat.id, final_text, entities=final_entities, reply_markup=markup,
                )
            except TelegramError:
                pass
            return

        # The free-text AI classifier is reserved for owner-tier (bot owner,
        # or whoever's been granted owner access to this group) -- regular
        # admins get redirected to the actual slash commands instead of
        # having their free text run through AI classification.
        is_owner_tier = is_owner(user.id) or await asyncio.to_thread(has_group_access, chat.id, user.id)
        if not is_owner_tier:
            try:
                await context.bot.send_message(
                    chat.id,
                    stylize("ℹ️ <b>Iske liye normal command use karo</b> (jaise /ban, /mute, /warn, /promote, /id) -- "
                    "yeh wali free-text ('..') sirf group owner ke liye hai."),
                    parse_mode=ParseMode.HTML,
                )
            except TelegramError:
                pass
        ...
        actions_list = result.get("actions") if isinstance(result.get("actions"), list) and result.get("actions") else [result]
        reason = result.get("reason") or "Admin request"

        # Deterministic safety-net: "yahan tak" / "up to here" style phrasing
        # always means a RANGE purge, never a single-message delete -- don't
        # leave this to the AI's judgment alone (it can misread "delete" in
        # "yahan tak delete karo" as the single-message action).
        if has_reply and re.search(r"yaha[nm]?\s*tak|is\s*(se|tak)\s*yaha|up\s*to\s*here|till\s*here", command_text, re.IGNORECASE):
            for a in actions_list:
                if a.get("action") == "delete":
                    a["action"] = "purge"

        # Capture every reply the actions make (instead of each sending its
        # own message) so it can all be combined into ONE clean message.
        captured = []
        for single_action in actions_list:
            await execute_admin_action(context, update, single_action, user, command_text, capture=captured)

        body = "\n".join(captured)

        # The full "Requested by / Target / Command / Reason" header is only
        # worth showing for real person-targeted admin-management actions
        # (ban, mute, warn, promote, etc.) -- for everything else (delete,
        # purge, pin, lock...) it was showing up as noise on top of a
        # response that's already self-explanatory (or, for delete/purge,
        # nothing at all). Those just get their own short bold line(s),
        # no header.
        if any(a.get("action") in ADMIN_HEADER_ACTIONS for a in actions_list):
            header_target = None
            if msg.reply_to_message:
                header_target = msg.reply_to_message.from_user
            else:
                uname_match = re.search(r"@([A-Za-z0-9_]{4,})", command_text)
                if uname_match:
                    header_target = await find_user_by_username(uname_match.group(1), chat.id, context)
            admin_link = f"<a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>"
            target_link = (
                f"<a href='tg://user?id={header_target.id}'>{md_escape(header_target.first_name)}</a>"
                if header_target else "N/A"
            )
            command_labels = ", ".join(str(a.get("action", "unknown")).replace("_", " ").title() for a in actions_list)
            header = (
                f"👤 Requested by: {admin_link}\n"
                f"🎯 Target: {target_link}\n"
                f"📋 Command: {command_labels}\n"
                f"📝 Reason: {md_escape(reason)}\n"
            )
            final_text_source = f"{header}\n{body}" if body else header
        else:
            final_text_source = body

        if not final_text_source.strip():
            return  # e.g. delete/purge already showed (or intentionally skipped) their own confirmation

        final_text, final_entities = render_with_emojis(final_text_source)
        try:
            markup = await owner_credit_button(context)
            await context.bot.send_message(chat.id, final_text, entities=final_entities, reply_markup=markup)
        except TelegramError:
...
# =============================================================================
async def mute_user(context, chat_id, user_id, minutes=None):
    """minutes=None -> permanent mute (no until_date at all)."""
    ...
    try:
        await context.bot.restrict_chat_member(chat_id, user_id, **kwargs)
    except TelegramError as e:
    ...
    try:
        await context.bot.restrict_chat_member(
            ...
            permissions=ChatPermissions(
                ...
                can_send_audios=True, can_send_documents=True, can_send_other_messages=True,
                can_send_polls=True, can_send_voice_notes=True, can_send_video_notes=True,
...
# Only these get the full "Requested by / Target / Command / Reason" header
# in the ".." command response -- real person-targeted admin-management
# actions. Everything else (delete, purge, pin, unpin, lock, unlock, info...)
# just gets its own short bold line(s), no header.
ADMIN_HEADER_ACTIONS = {"ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
                         "promote", "demote", "escrow_verify", "escrow_unverify",
                         "upgrade_trust", "downgrade_trust", "dispute_freeze",
                         "dispute_unfreeze", "grant_permission", "toggle_permission",
                         "set_title"}

# Rotates so it's not the same line every time. {name} gets the clickable
# short-name link substituted in.
_NOT_ADMIN_ROASTS = [
    "😂 <b>{name}, admin ka sapna abhi sapna hi rahega.</b>",
    "🙄 <b>{name}, itni himmat? Member ho, boss nahi.</b>",
    "🤡 <b>{name}, command chalane se pehle power toh le aa.</b>",
    "😹 <b>{name} ne khud ko group ka malik samajh liya kya.</b>",
    "😏 <b>{name}, permission zero hai bhai, attitude full hai.</b>",
    "💀 <b>{name}, tu member hai, manager nahi -- baith ja.</b>",
    "🫠 <b>{name} ka authority level bilkul guest jaisa hai.</b>",
]

PERMISSION_FIELD_MAP = {
    "messages": "can_send_messages", "text": "can_send_messages",
    "photos": "can_send_photos", "photo": "can_send_photos",
    "videos": "can_send_videos", "video": "can_send_videos",
    "audios": "can_send_audios", "audio": "can_send_audios", "music": "can_send_audios",
    "documents": "can_send_documents", "files": "can_send_documents", "document": "can_send_documents",
    "stickers_gifs": "can_send_other_messages", "stickers": "can_send_other_messages",
    "gifs": "can_send_other_messages", "animations": "can_send_other_messages",
    "polls": "can_send_polls", "poll": "can_send_polls",
    "voice_notes": "can_send_voice_notes", "voice": "can_send_voice_notes",
    "video_notes": "can_send_video_notes",
    "link_previews": "can_add_web_page_previews", "web_previews": "can_add_web_page_previews",
    "invite_users": "can_invite_users", "invite": "can_invite_users",
...
async def apply_warn_and_maybe_ban(context, chat_id, user_id, reason):
    """Adds a warn; if the user has now hit this group's configured warn
    limit, applies this group's configured warn_action (ban/kick/mute) and
    clears their warn history. Returns (count, actioned: bool)."""
    ...
    if count >= limit:
        try:
            if action == "kick":
                ...
                await mute_user(context, chat_id, user_id)  # permanent, until manually unmuted
            else:  # default: ban
                await context.bot.ban_chat_member(chat_id, user_id)
                await asyncio.to_thread(record_ban, user_id, chat_id, f"Auto-banned: {limit} warnings reached")
        except TelegramError as e:
            logger.warning("auto-%s after warn limit failed: %s", action, e)
...
async def purge_message_range(bot, chat_id: int, message_ids: list) -> int:
    """Deletes every id in message_ids as fast as Telegram allows: bulk
    delete_messages() in chunks of 100 (the API's hard per-call limit), with
    a per-message delete_message() fallback ONLY if the whole chunk raises
    (e.g. some ids in it are unreachable). No sender filtering anywhere --
    user, admin, owner, or bot, every id is targeted the same way.

    CAVEAT (real Telegram API limitation, not a bug in this code): a bulk
    delete_messages() call that does NOT raise is assumed to have deleted
    every id in that chunk -- but Telegram's bulk endpoint only ever
    returns a single True/False for the whole batch, never a per-id
    result, and it can silently skip ids it can't delete without raising
    anything. So the count this returns is "how many we attempted without
    an error", not an independently verified "how many are confirmed
    gone" -- those can differ. If you need a guaranteed-accurate count,
    that only comes from deleting one id at a time (slower, no batching)."""
    deleted = 0
    ...
    for chunk in chunks:
        try:
            ...
            logger.warning("purge: bulk delete_messages failed for a chunk (%s), falling back to individual deletes.", e)
            for mid in chunk:
                try:
                    await bot.delete_message(chat_id=chat_id, message_id=mid)
                    deleted += 1
                except TelegramError:
                    pass  # already gone / too old / never existed -- skip and keep going
        await asyncio.sleep(0.3)  # stay under Telegram's flood limits between chunks
    return deleted


async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user, command_text: str = "", capture: list = None):
    msg = update.effective_message

    async def _reply(text_content, **kwargs):
        # When capture is provided (multi-command batch mode), collect
        # the text instead of sending immediately -- the caller combines
        # everything into ONE final message. Otherwise behaves exactly
        # like msg.reply_text always did.
        if capture is not None:
            capture.append(text_content)
            return None
        return await msg.reply_text(text_content, **kwargs)
    chat = update.effective_chat
    ...
    if action == "unknown" or not action:
        ...
        await _reply(stylize(answer))
        return

    if action in OWNER_ONLY_ACTIONS and not (is_owner(admin_user.id) or has_group_access(chat.id, admin_user.id)):
        await _reply(stylize("🔒 <b>Yeh command sirf group owner use kar sakta hai.</b>"), parse_mode=ParseMode.HTML)
        return

    if not await check_permission_or_explain(context, chat, admin_user, action):
        return
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            # Safety net: a destructive action ending up targeting the exact
            # admin who issued the command is essentially always a
            # misclassification (e.g. they were replying to their OWN
            # message for something unrelated, like setting a filter, and
            # the free-text got misread as an action against themselves) --
            # never actually a real intent. Block it outright instead of
            # executing it.
            if action in ("ban", "kick", "mute", "warn") and target.id == admin_user.id:
                await _reply(
                    stylize("🤨 <b>Khud pe yeh action nahi lagega — shayad kuch aur bolna chah rahe the?</b>"),
                    parse_mode=ParseMode.HTML,
                )
                return
            ...
            if action == "ban":
                ...
                await _reply(stylize(f"🔨 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ban kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
                ...
                await _reply(stylize(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> unban kar diya gaya!</b>"), parse_mode=ParseMode.HTML)

            elif action == "kick":
                target_member = await get_admin_member(context.bot, chat.id, target.id)
                is_target_admin = target_member is not None and target_member.status in (ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER)
                if is_target_admin and not await is_full_group_owner(context.bot, chat.id, admin_user.id):
                    await _reply(
                        stylize(f"🔒 <b>Main kisi admin ko kick nahi kar sakta</b> — sirf group ka owner yeh kar sakta hai."),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    await context.bot.ban_chat_member(chat.id, target.id)
                    await context.bot.unban_chat_member(chat.id, target.id)
                    await _reply(stylize(f"👢 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> kick kar diya gaya!</b>"), parse_mode=ParseMode.HTML)

            elif action == "mute":
                await mute_user(context, chat.id, target.id, duration)
                if duration:
                    await _reply(stylize(f"🔇 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> {duration} min ke liye mute!</b>"), parse_mode=ParseMode.HTML)
                else:
                    await _reply(stylize(f"🔇 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> permanently mute kar diya gaya</b> (jab tak manually unmute na ho)!"), parse_mode=ParseMode.HTML)

            elif action == "unmute":
                await unmute_user(context, chat.id, target.id)
                await _reply(stylize(f"🔊 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> unmute kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
                ...
                if banned:
                    await _reply(
                        ...
                        f"warning-limit paar karne ke baad action liya gaya (group setting ke hisaab se).</b>"), parse_mode=ParseMode.HTML,
                    )
                else:
                    label = f"{add_count} warnings" if add_count > 1 else "1 warning"
                    await _reply(stylize(f"⚠️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ko {label} diye gaye! (Total: {count})</b>"), parse_mode=ParseMode.HTML)
                ...
                if current_count == 0:
                    ...
                    await _reply(stylize(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ki {label} hata di gayi! (Baaki: {new_count})</b>"), parse_mode=ParseMode.HTML)

            elif action == "pin":
                if not msg.reply_to_message:
                    await _reply(stylize("↩️ <b>Jo message pin karna hai, use reply karke command do.</b>"), parse_mode=ParseMode.HTML)
                else:
                    await context.bot.pin_chat_message(chat.id, msg.reply_to_message.message_id)
                    await _reply(stylize("📌 <b>Message pin kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
                ...
                await _reply(stylize("📌 <b>Unpin kar diya gaya!</b>"), parse_mode=ParseMode.HTML)

            elif action == "delete":
                if not msg.reply_to_message:
                    await _reply(stylize("↩️ <b>Jo message delete karna hai, use reply karke command do.</b>"), parse_mode=ParseMode.HTML)
                else:
                    await context.bot.delete_message(chat.id, msg.reply_to_message.message_id)
                    # The admin's own command message ("is message ko delete
                    # karo" / "/del") should disappear too, not just the
                    # target -- delete it last so nothing else here depends
                ...
                await context.bot.promote_chat_member(
                    chat.id, target.id, is_anonymous=False,
                    can_manage_chat=False, can_delete_messages=True, can_manage_video_chats=False,
                ...
                verify_member = await context.bot.get_chat_member(chat.id, target.id)
                if verify_member.status != ChatMemberStatus.ADMINISTRATOR:
                    await _reply(
                        stylize(f"⚠️ <b>Command Telegram ko bhej diya, lekin verify karne par "
                        f"<a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> abhi bhi admin "
                        f"nahi dikh raha.</b> Bot ke paas is group me 'Add New Admins' ki permission check kar lo."),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    _admin_member_cache.pop((chat.id, target.id), None)  # force-refresh cached rights
                    await asyncio.to_thread(record_promotion, chat.id, target.id, admin_user.id)
                    _pending_permission_grant[(chat.id, admin_user.id)] = (target.id, time.time())
                    await _reply(
                        stylize(f"⭐ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> "
                        f"admin ban gaya!</b>"),
                        parse_mode=ParseMode.HTML,
                    )

            elif action == "demote":
                allowed = await is_full_group_owner(context.bot, chat.id, admin_user.id)
                if not allowed:
                    promoter_id = await asyncio.to_thread(get_promoter, chat.id, target.id)
                    allowed = promoter_id == admin_user.id
                if not allowed:
                    await _reply(
                        stylize(f"🔒 <b>Sirf group ka owner, ya jisne <a href='tg://user?id={target.id}'>"
                        f"{md_escape(target.first_name)}</a> ko khud promote kiya tha, use hi demote kar sakta hai.</b>"),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    ...
                    await asyncio.to_thread(clear_promotion_record, chat.id, target.id)
                    await _reply(stylize(f"⬇️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> demote kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
                ...
                if not allowed:
                    await _reply(
                        stylize(f"🔒 <b>Sirf group ka owner, ya jisne <a href='tg://user?id={target.id}'>"
                        f"{md_escape(target.first_name)}</a> ko khud promote kiya tha, use hi extra "
                        f"permissions de sakta hai.</b>"),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    numbers = set(result.get("permission_numbers") or [])
                    rights_map = {
                        1: "can_delete_messages", 2: "can_restrict_members", 3: "can_pin_messages",
                        4: "can_invite_users", 5: "can_manage_video_chats", 6: "can_change_info",
                        7: "can_promote_members",
                    ...
                    if not grant_kwargs:
                        await _reply(
                            stylize("🤔 <b>Kaunse number/permission dene hain, clearly bolo.</b>\n\n"
                            "1. Delete messages\n2. Restrict/ban/mute members\n3. Pin messages\n"
                            "4. Invite users via link\n5. Manage video chats\n6. Change group info\n"
                            "7. Add new admins (promote others)"),
                            parse_mode=ParseMode.HTML,
                        )
                    else:
                        target_member = await get_admin_member(context.bot, chat.id, target.id)
                        current_rights = {f: getattr(target_member, f, False) for f in rights_map.values()} if target_member else {}
                        current_rights.update(grant_kwargs)
                        current_rights["is_anonymous"] = False
                        await context.bot.promote_chat_member(chat.id, target.id, **current_rights)
                        granted_labels = ", ".join(rights_map[k].replace("can_", "").replace("_", " ").title() for k in sorted(numbers) if k in rights_map)
                        await _reply(
                            stylize(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ko extra "
                            f"permissions mil gaye:</b> {granted_labels}"),
                            parse_mode=ParseMode.HTML,
                        )
                ...
                await _reply(stylize(f"💎 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> escrow-verified!</b>"), parse_mode=ParseMode.HTML)
                ...
                await _reply(stylize(f"❌ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ka escrow-verified badge hata diya gaya.</b>"), parse_mode=ParseMode.HTML)
                ...
                await asyncio.to_thread(set_trust_level, target.id, lvl)
                await _reply(stylize(f"⬇️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> downgrade kar diya gaya — "
                    f"{TRUST_LEVELS[lvl]}</b>"), parse_mode=ParseMode.HTML)
                ...
                await _reply(stylize("✅ <b>Dispute unfreeze kar diya gaya.</b>"), parse_mode=ParseMode.HTML)
                ...
                bracket_match = re.search(r"\(([^)]+)\)", command_text)
                if target.id == OWNER_ID:
                    await _reply(
                        stylize("🔒 <b>Apna khud ka tag/title change nahi kar sakte — sirf doosron ka.</b>"),
                        parse_mode=ParseMode.HTML,
                    )
                elif not bracket_match:
                    await _reply(
                        stylize("🤔 <b>Naya title bracket () ke andar likho.</b>\nExample: <code>iska naam change karo (SR) rakh do</code>"),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    new_title = bracket_match.group(1).strip()[:16]
                    try:
                        try:
                            await context.bot.set_chat_administrator_custom_title(chat.id, target.id, new_title)
                        except BadRequest:
                            # If this ran right after a "promote" in the same
                            # command, Telegram's own backend can take a beat
                            # to finish propagating the new admin status --
                            # setting the custom title too fast then fails
                            # even though the promotion itself already went
                            # through. One short retry covers that race
                            # instead of reporting a false "not an admin" error.
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            if not msg.reply_to_message:
                await _reply(stylize("↩️ <b>Jahan tak purge karna hai us message ko reply karo.</b>"), parse_mode=ParseMode.HTML)
                return
            start_id = msg.reply_to_message.message_id
            end_id = msg.message_id
            message_ids = list(range(start_id, end_id + 1))
            deleted = await purge_message_range(context.bot, chat.id, message_ids)

            await asyncio.to_thread(log_admin_action, admin_user.id, admin_user.first_name, "purge", 0, f"{deleted} messages")
            try:
                status_msg = await context.bot.send_message(
                    chat.id, stylize(f"🧹 <b>{deleted} messages purge kar diye gaye.</b>"), parse_mode=ParseMode.HTML,
                )
            ...
            await context.bot.set_chat_permissions(chat.id, ChatPermissions(can_send_messages=False), use_independent_chat_permissions=True)
            await _reply(stylize("🔒 <b>Chat lock kar diya gaya — sirf admins bhej sakte hain.</b>"), parse_mode=ParseMode.HTML)
            ...
            await _reply(stylize("🔓 <b>Chat unlock kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
            await asyncio.to_thread(log_admin_action, admin_user.id, admin_user.first_name, "unlock", 0, "")

        elif action == "toggle_permission":
            perm_key = PERMISSION_FIELD_MAP.get(str(result.get("permission") or "").lower())
            enabled = bool(result.get("enabled", False))
            if not perm_key:
                await _reply(stylize("🤔 <b>Samajh nahi aaya kaunsi setting badalni hai.</b>"), parse_mode=ParseMode.HTML)
            else:
                bot_member = await context.bot.get_chat_member(chat.id, context.bot.id)
                if not getattr(bot_member, "can_restrict_members", False):
                    await _reply(
                        stylize("❌ <b>Yeh badal nahi paya — bot ke paas is group me 'Restrict Members' "
                        "admin-permission nahi hai.</b> Group ke owner se bot ko yeh permission dilwao, "
                        "tabhi group ki settings (photo/video/link/etc) badli ja sakti hain."),
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    current_chat = await context.bot.get_chat(chat.id)
                    current_perms = current_chat.permissions or ChatPermissions()
                    new_perms_kwargs = {
                        f: getattr(current_perms, f, False)
                        for f in (
                            "can_send_messages", "can_send_photos", "can_send_videos", "can_send_audios",
                            "can_send_documents", "can_send_other_messages", "can_send_polls",
                            "can_send_voice_notes", "can_send_video_notes", "can_add_web_page_previews",
                            "can_invite_users", "can_pin_messages", "can_change_info",
                        )
                    }
                    new_perms_kwargs[perm_key] = enabled
                    await context.bot.set_chat_permissions(
                        chat.id, ChatPermissions(**new_perms_kwargs), use_independent_chat_permissions=True
                    )

                    # VERIFY -- never just assume the API call did what we asked.
                    # Re-fetch the chat and confirm the field actually reflects
                    # the change before telling the admin it worked.
                    verify_chat = await context.bot.get_chat(chat.id)
                    verify_perms = verify_chat.permissions
                    actually_set = bool(getattr(verify_perms, perm_key, None)) == enabled if verify_perms else False
                    perm_label = str(result.get("permission")).replace("_", " ").title()

                    if actually_set:
                        await _reply(
                            stylize(f"✅ <b>Verified:</b> <b>{perm_label}</b> ab is group me "
                            f"<b>{'ALLOWED' if enabled else 'BLOCKED'}</b> hai — baaki saari settings waisi hi rahi jaisi thi.\n\n"
                            f"<i>Note: yeh sirf normal members pe lagu hota hai — group admins Telegram ke "
                            f"niyam ke hisaab se kabhi bhi in restrictions se exempt hote hain, chahe kitne "
                            f"bhi permissions block ho.</i>"),
                            parse_mode=ParseMode.HTML,
                        )
                        await asyncio.to_thread(log_admin_action, admin_user.id, admin_user.first_name, "toggle_permission", 0, str(result.get("permission")))
                    else:
                        await _reply(
                            stylize(f"⚠️ <b>Command Telegram ko bhej diya, lekin verify karne par change reflect "
                            f"nahi hua.</b> Yeh tab hota hai jab group ki koi upar-level restriction (jaise "
                            f"Telegram ka apna anti-spam mode) isse override kar rahi ho. Group Settings > "
                            f"Permissions me jaakar khud ek baar manually check kar lo."),
                            parse_mode=ParseMode.HTML,
                        )

    except BadRequest as e:
        # Never dump a raw stack trace/technical exception into the group --
        # but Telegram's own BadRequest message text IS usually a genuine,
        # human-readable, accurate reason (e.g. "Not enough rights to
        # restrict/unrestrict chat member", "CHAT_ADMIN_INVITE_REQUIRED"),
        # so surface THAT instead of a vague guess. Full detail also DMed
        # to the owner.
        logger.warning("execute_admin_action BadRequest: %s", e)
        await _reply(
            stylize(f"❌ <b>Yeh action nahi ho paya.</b>\n<i>Telegram ka reason:</i> <code>{md_escape(str(e))}</code>\n"
            "(Bot ke paas full permissions hain, par Telegram kabhi-kabhi kisi specific admin-hierarchy "
            "rule ki wajah se bhi block kar deta hai -- jaise sirf usi ko demote kiya ja sakta hai jisko "
            "khud promote kiya ho.)"),
            parse_mode=ParseMode.HTML,
        )
        try:
...
async def send_admin_log_dm(context, admin_user, action, target):
    ...
    if admin_user.id == OWNER_ID or target is None:
        return
    try:
        await context.bot.send_message(
            OWNER_ID,
...
def get_display_status(chat_id, user_id, db_trust_level, db_escrow_verified):
    """A user granted access to manage a group (via /grant) IS that group's
    owner for display purposes -- shows as Owner/max-trust/escrow-verified
    regardless of their actual DB row. Returns (status_label, escrow_label)."""
    if has_group_access(chat_id, user_id):
        return "👑 Owner", "💎 Yes"
    return TRUST_LEVELS.get(db_trust_level, TRUST_LEVELS[0]), ("💎 Yes" if db_escrow_verified else "❌ No")
...
async def send_user_info(context, chat_id, target):
    ...
    trust = row["trust_level"] if row else 0
    join_date = row["join_date"][:10] if row and row["join_date"] else "N/A"
    status_label, escrow = get_display_status(chat_id, target.id, trust, row["escrow_verified"] if row else 0)
    ...
    text = (
        ...
        f"🔗 Profile: <a href='{user_profile_link(target.id, target.username)}'>{md_escape(target.first_name)}</a>\n"
        f"📅 Joined: {join_date}\n"
        f"⚠️ Warnings: {warns}\n"
        f"💎 Escrow Verified: {escrow}\n"
        f"🏆 <b>Status in GC:</b> {status_label}"
    )
...
async def user_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/info -- reply to someone for their info, mention @username for
    theirs, or just send /info alone to see your OWN info."""
    msg = update.effective_message
    chat = update.effective_chat
    if msg.reply_to_message:
        ...
        text = msg.text or ""
        uname_match = re.search(r"@([A-Za-z0-9_]{4,})", text)
        if uname_match and chat.type != "private":
            target = await find_user_by_username(uname_match.group(1), chat.id, context)
            if not target:
                await msg.reply_text(stylize(f"❌ <b>@{md_escape(uname_match.group(1))} nahi mila.</b>"), parse_mode=ParseMode.HTML)
                return
        else:
            target = update.effective_user  # no reply, no @username -> show sender's OWN info
...
async def welcome_toggle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user is None or not await is_user_admin(context.bot, chat.id, user.id):
        return
    parts = (update.effective_message.text or "").split()
    if len(parts) != 2 or parts[1].lower() not in ("on", "off"):
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/welcome on</code> ya <code>/welcome off</code>"), parse_mode=ParseMode.HTML)
        return
    enabled = 1 if parts[1].lower() == "on" else 0
    await asyncio.to_thread(set_group_setting, chat.id, "welcome_enabled", enabled)
    await update.effective_message.reply_text(
        stylize("✅ <b>Welcome message ON kar diya gaya!</b>" if enabled else "🔕 <b>Welcome message OFF kar diya gaya.</b>"),
        parse_mode=ParseMode.HTML,
    )


async def welcome_image_toggle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/welcomeimage on|off -- controls ONLY the new member's own profile
    photo attaching to their welcome message. A custom welcome image/video
    set via the panel always overrides this and shows regardless."""
    chat = update.effective_chat
    if chat.type == "private":
        return
    user = update.effective_user
    ...
    parts = (update.effective_message.text or "").split()
    if len(parts) != 2 or parts[1].lower() not in ("on", "off"):
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/welcomeimage on</code> ya <code>/welcomeimage off</code>"), parse_mode=ParseMode.HTML)
        return
    enabled = 1 if parts[1].lower() == "on" else 0
    await asyncio.to_thread(set_group_setting, chat.id, "welcome_profile_pic_enabled", enabled)
    await update.effective_message.reply_text(
        stylize("🖼️ <b>Naye member ki profile photo welcome message ke saath ab dikhegi.</b>" if enabled
                else "🚫 <b>Naye member ki profile photo ab welcome message ke saath NAHI dikhegi.</b>"),
        parse_mode=ParseMode.HTML,
    )


async def addblock_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if chat.type == "private":
        return
    user = update.effective_user
    ...
    word = text.split(None, 1)[1].strip() if len(text.split(None, 1)) > 1 else ""
    if not word:
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/addblock &lt;word/phrase&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(add_blacklist_word, chat.id, word, user.id)
    await update.effective_message.reply_text(stylize(f"✅ <b>'{md_escape(word)}' blacklist me add ho gaya.</b>"), parse_mode=ParseMode.HTML)


async def rmblock_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if chat.type == "private":
        return
    user = update.effective_user
    ...
    word = text.split(None, 1)[1].strip() if len(text.split(None, 1)) > 1 else ""
    if not word:
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/rmblock &lt;word/phrase&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    removed = await asyncio.to_thread(remove_blacklist_word, chat.id, word)
    await update.effective_message.reply_text(
        stylize(f"✅ <b>'{md_escape(word)}' hata diya gaya.</b>" if removed else "ℹ️ <b>Yeh word blacklist me tha hi nahi.</b>"),
        parse_mode=ParseMode.HTML,
    )
...
async def blocklist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private":
        return
    words = await asyncio.to_thread(get_blacklist_words, chat.id)
    if not words:
        await update.effective_message.reply_text(stylize("😶 <b>Is group me koi blacklisted word nahi hai.</b>"), parse_mode=ParseMode.HTML)
        return
    lines = ["🚫 <b>BLACKLISTED WORDS</b> 🚫\n"] + [f"• <code>{md_escape(w)}</code>" for w in words]
...
async def setwarnlimit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2 or not parts[1].isdigit() or int(parts[1]) < 1:
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/setwarnlimit &lt;number&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat.id, "warn_limit", int(parts[1]))
    await update.effective_message.reply_text(stylize(f"✅ <b>Warn limit set: {parts[1]}</b>"), parse_mode=ParseMode.HTML)
...
async def setwarnaction_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) != 2 or parts[1].lower() not in ("ban", "kick", "mute"):
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/setwarnaction &lt;ban|kick|mute&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat.id, "warn_action", parts[1].lower())
    await update.effective_message.reply_text(stylize(f"✅ <b>Warn-limit action set:</b> {parts[1].lower()}"), parse_mode=ParseMode.HTML)


async def _resolve_mod_settings_target(update: Update, context: ContextTypes.DEFAULT_TYPE, cmd_name: str, value_desc: str):
    """Shared by the abuse/link mute+warn settings commands below -- works
    BOTH in-group (chat implicit, admin-of-that-group required) AND in DM
    (group id given as the first argument, same admin-of-that-group check,
    just done explicitly since DM has no implicit chat). Returns
    (chat_id, value_str) on success, or None after already sending the
    person an error/usage reply."""
    chat = update.effective_chat
    user = update.effective_user
    msg = update.effective_message
    ...
    usage = (
        f"ℹ️ <b>Use (DM):</b> <code>/{cmd_name} &lt;group_id&gt; &lt;{value_desc}&gt;</code>"
        if in_dm else
        f"ℹ️ <b>Use:</b> <code>/{cmd_name} &lt;{value_desc}&gt;</code>"
    )
    if in_dm:
        if len(parts) != 3:
            await msg.reply_text(stylize(usage), parse_mode=ParseMode.HTML)
            return None
        try:
            chat_id = int(parts[1])
        except ValueError:
            await msg.reply_text(stylize("❌ <b>Group ID ek number hona chahiye.</b>"), parse_mode=ParseMode.HTML)
            return None
        value = parts[2]
    else:
        if len(parts) != 2:
        ...
        chat_id = chat.id
        value = parts[1]
    if user is None or not await is_user_admin(context.bot, chat_id, user.id):
        await msg.reply_text(stylize("🔒 <b>Us group mein admin nahi ho.</b>"), parse_mode=ParseMode.HTML)
        return None
    return chat_id, value


async def setabusemute_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    resolved = await _resolve_mod_settings_target(update, context, "setabusemute", "minutes|off")
    if not resolved:
        return
    chat_id, value = resolved
    if value.lower() == "off":
        minutes = 0
    elif value.isdigit() and int(value) > 0:
        minutes = int(value)
    else:
        await update.effective_message.reply_text(stylize("ℹ️ <b>Minutes ek positive number ho, ya 'off'.</b>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat_id, "abuse_mute_min", minutes)
    label = "OFF (mute nahi hoga)" if minutes == 0 else f"{minutes} minute"
    await update.effective_message.reply_text(stylize(f"✅ <b>Gaali dene par mute duration set:</b> {label}"), parse_mode=ParseMode.HTML)


async def setlinkmute_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    resolved = await _resolve_mod_settings_target(update, context, "setlinkmute", "minutes|off")
    if not resolved:
        return
    chat_id, value = resolved
    if value.lower() == "off":
        ...
        await update.effective_message.reply_text(stylize("ℹ️ <b>Minutes ek positive number ho, ya 'off'.</b>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat_id, "link_mute_min", minutes)
    label = "OFF (mute nahi hoga)" if minutes == 0 else f"{minutes} minute"
    await update.effective_message.reply_text(stylize(f"✅ <b>Link/promotion bhejne par mute duration set:</b> {label}"), parse_mode=ParseMode.HTML)


async def setabusewarn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    resolved = await _resolve_mod_settings_target(update, context, "setabusewarn", "on|off")
    if not resolved:
        return
    chat_id, value = resolved
    if value.lower() not in ("on", "off"):
        await update.effective_message.reply_text(stylize("ℹ️ <b>'on' ya 'off' likho.</b>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat_id, "abuse_warn_enabled", 1 if value.lower() == "on" else 0)
    await update.effective_message.reply_text(stylize(f"✅ <b>Gaali dene par warning:</b> {value.upper()}"), parse_mode=ParseMode.HTML)


async def setlinkwarn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    resolved = await _resolve_mod_settings_target(update, context, "setlinkwarn", "on|off")
    if not resolved:
        return
    chat_id, value = resolved
    if value.lower() not in ("on", "off"):
        await update.effective_message.reply_text(stylize("ℹ️ <b>'on' ya 'off' likho.</b>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat_id, "link_warn_enabled", 1 if value.lower() == "on" else 0)
    await update.effective_message.reply_text(stylize(f"✅ <b>Link/promotion bhejne par warning:</b> {value.upper()}"), parse_mode=ParseMode.HTML)


async def modsettings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Shows this group's current abuse/link/warn moderation settings --
    works in-group (implicit) or in DM as /modsettings <group_id>."""
    chat = update.effective_chat
    user = update.effective_user
    msg = update.effective_message
    if chat.type == "private":
        parts = (msg.text or "").split()
        if len(parts) != 2:
            await msg.reply_text(stylize("ℹ️ <b>Use (DM):</b> <code>/modsettings &lt;group_id&gt;</code>"), parse_mode=ParseMode.HTML)
            return
        try:
            chat_id = int(parts[1])
        except ValueError:
    ...
    limit, action = await asyncio.to_thread(get_warn_settings, chat_id)
    lines = [
        "⚙️ <b>MODERATION SETTINGS</b> ⚙️\n",
        f"🤬 Gaali -> mute: {(str(abuse_mute) + ' min') if abuse_mute else 'OFF'}, warn: {'ON' if abuse_warn else 'OFF'}",
        f"🔗 Link/promotion -> mute: {(str(link_mute) + ' min') if link_mute else 'OFF'}, warn: {'ON' if link_warn else 'OFF'}",
        f"⚠️ Warn-limit: {limit} (action: {action})",
    ]
    await msg.reply_text(stylize("\n".join(lines)), parse_mode=ParseMode.HTML)


def id_reply_text(chat, user) -> str:
    """Shared by /id and the plain-text 'id' trigger."""
    if chat.type == "private":
        return f"🆔 <b>Your ID:</b> <code>{user.id}</code>"
    return f"🆔 <b>Group ID:</b> <code>{chat.id}</code>\n🆔 <b>Your ID:</b> <code>{user.id}</code>"
...
async def id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user is None:
        return
    try:
        await update.effective_message.reply_text(stylize(id_reply_text(chat, user)), parse_mode=ParseMode.HTML)
    except TelegramError:
        pass


async def apisetup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Bot-owner-only, DM-only. Starts a 1-message flow via the SAME
    panel_state mechanism the rest of the DM panel uses (see
    dm_text_handler) -- reply here with the raw key next, it gets saved to
    the DB and the message deleted. Run this command again any time to add
    ANOTHER key -- no limit, every active key is used in rotation (see
    _ai_call_raw)."""
    user = update.effective_user
    chat = update.effective_chat
    if user is None or not is_owner(user.id):
        return  # silent -- only the bot owner manages AI keys
    if chat.type != "private":
        try:
            await update.effective_message.reply_text(
                stylize("🔒 <b>Mujhe DM mein /apisetup bhejo</b> — API key yahan group mein share mat karna."),
                parse_mode=ParseMode.HTML,
            )
        except TelegramError:
            pass
        return
    context.user_data["panel_state"] = AWAITING_API_KEY
    try:
        await update.effective_message.reply_text(
            stylize("🔑 <b>Ab apna OpenRouter API key bhejo</b> (agli message mein, seedha yahan DM pe).\n"
            "Bhejte hi save ho jayegi aur wo message turant delete kar dunga."),
            parse_mode=ParseMode.HTML,
        )
    except TelegramError:
        pass
...
RULES_BUTTON_EMOJI_ID = "6129746001654718223"


async def rules_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    chat = update.effective_chat
    gs = await asyncio.to_thread(get_group_settings, chat.id)
    rules = gs.get("rules_text")
    if not rules:
        await query.answer("📜 Koi rules set nahi hain abhi.", show_alert=True)
        return
    # The rules text goes DIRECTLY in the popup itself now -- not DM'd.
    # Telegram's callback-answer alert is plain text only (no HTML, no
    # entities -- so no bold/premium-emoji is possible here, and it has a
    # hard 200-character cap). If the rules are short (which is the whole
    # point of putting them here instead of a DM), they show in full.
    popup_text = f"📜 RULES 📜\n\n{rules}"
...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rules = gs.get("rules_text")
    if not rules:
        await update.effective_message.reply_text(stylize("😶 <b>Is group ke liye abhi koi rules set nahi hain.</b>"), parse_mode=ParseMode.HTML)
        return
    final_text, final_entities = render_with_emojis(f"📜 <b>GROUP RULES</b> 📜\n\n{rules}")
...
async def set_rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user is None or not await is_user_admin(context.bot, chat.id, user.id):
        return
    text = (update.effective_message.text or "")
    rules_body = text.split(None, 1)[1] if len(text.split(None, 1)) > 1 else ""
    if not rules_body.strip():
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/setrules &lt;rules text&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    await asyncio.to_thread(set_group_setting, chat.id, "rules_text", rules_body.strip())
    await update.effective_message.reply_text(stylize("✅ <b>Rules set ho gaye!</b> <code>/rules</code> se koi bhi dekh sakta hai."), parse_mode=ParseMode.HTML)
...
async def group_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "wtext":
        ...
        await context.bot.send_message(
            ...
            stylize("↩️ <b>Isko copy-edit karke seedha yahan bhej do, set ho jayega — bilkul waisa hi "
            "(font/style bhi) jaisa aap bhejoge, koi auto-change nahi hoga.</b>\n\n"
            ...
            "🖼️ <b>Image/Video ke saath chahiye?</b> Photo/video bhejo, caption me hi welcome "
            "text likh do — dono ek saath set ho jayenge (member ki apni profile pic ki jagah "
            "yeh fix image/video hamesha dikhega).\n\n"
            "🎨 Premium emoji seedha type/paste karke bhejo — khud-ba-khud save + set ho jayega, "
            "koi placeholder nahi lagana.\n\n"
            "🔕 Welcome band karna hai? Group me <code>/welcome off</code> likho (<code>/welcome on</code> se wapas)."),
            parse_mode=ParseMode.HTML,
        )
    elif field == "button":
...
_DURATION_RE = re.compile(r"^(\d+)(m|min|h|hr|d)$", re.IGNORECASE)
...
async def _fast_slash_command(update: Update, context: ContextTypes.DEFAULT_TYPE, action: str, needs_target=True, needs_duration=False, needs_count=False):
    """Shared engine for the deterministic (non-AI) slash commands --
    reuses execute_admin_action directly so behavior/permission-checks stay
    100% consistent with the natural-language ".." path."""
    msg = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    ...
    if needs_duration and rest:
        tokens = rest.split(maxsplit=1)
        parsed = _parse_duration_token(tokens[0])
        if parsed is not None:
            duration_minutes = parsed
            rest = tokens[1].strip() if len(tokens) > 1 else ""

    count = 1
    if needs_count and rest:
        tokens = rest.split(maxsplit=1)
        if tokens[0].isdigit():
            count = int(tokens[0])
            rest = tokens[1].strip() if len(tokens) > 1 else ""

    if needs_target and msg.reply_to_message is None:
        uname_match = re.search(r"@([A-Za-z0-9_]{4,})", rest)
        if not uname_match:
            await msg.reply_text(stylize(f"↩️ <b>Kisi ko reply karke ya @username dekar /{action} use karo.</b>"), parse_mode=ParseMode.HTML)
            return
    ...
    result = {
        ...
        "count": count,
        "reason": rest if rest and not re.match(r"^@[A-Za-z0-9_]{4,}$", rest) else "Admin request",
        "confidence": "high",
    }
    await execute_admin_action(context, update, result, user, text)


async def cmd_ban(u, c): await _fast_slash_command(u, c, "ban")
async def cmd_unban(u, c): await _fast_slash_command(u, c, "unban")
async def cmd_kick(u, c): await _fast_slash_command(u, c, "kick")
async def cmd_mute(u, c): await _fast_slash_command(u, c, "mute", needs_duration=True)
async def cmd_unmute(u, c): await _fast_slash_command(u, c, "unmute")
async def cmd_warn(u, c): await _fast_slash_command(u, c, "warn", needs_count=True)
async def cmd_unwarn(u, c): await _fast_slash_command(u, c, "unwarn", needs_count=True)
async def cmd_admin(u, c): await _fast_slash_command(u, c, "promote")
async def cmd_unadmin(u, c): await _fast_slash_command(u, c, "demote")
async def cmd_pin(u, c): await _fast_slash_command(u, c, "pin")
async def cmd_unpin(u, c): await _fast_slash_command(u, c, "unpin")
async def cmd_del(u, c): await _fast_slash_command(u, c, "delete")
async def cmd_purge(u, c): await _fast_slash_command(u, c, "purge", needs_target=False)


async def cmd_tban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/tban <duration> [reason] -- temporary ban, auto-unbanned after duration."""
    msg = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    if chat.type == "private" or user is None:
    ...
    if msg.reply_to_message is None:
        await msg.reply_text(stylize("↩️ <b>Kisi ko reply karke /tban &lt;duration&gt; use karo.</b>"), parse_mode=ParseMode.HTML)
        return
    parts = (msg.text or "").split(maxsplit=2)
    if len(parts) < 2:
        await msg.reply_text(stylize("ℹ️ <b>Use:</b> <code>/tban 10m reason</code>"), parse_mode=ParseMode.HTML)
        return
    minutes = _parse_duration_token(parts[1])
    if minutes is None:
        await msg.reply_text(stylize("ℹ️ <b>Duration format galat hai.</b> Jaise: 10m, 2h, 1d"), parse_mode=ParseMode.HTML)
        return
    if not await check_permission_or_explain(context, chat, user, "ban"):
        return
    target = msg.reply_to_message.from_user
    try:
        await context.bot.ban_chat_member(chat.id, target.id, until_date=datetime.now(timezone.utc) + timedelta(minutes=minutes))
        await asyncio.to_thread(record_ban, target.id, chat.id, f"Temp-banned by {user.first_name} ({minutes}m)")
        await msg.reply_text(
            stylize(f"⏳ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> {minutes} min ke liye ban kar diya gaya</b> — auto-unban ho jayega."),
            parse_mode=ParseMode.HTML,
        )
    except TelegramError as e:
        await msg.reply_text(stylize(f"❌ <b>Failed:</b> <code>{md_escape(str(e))}</code>"), parse_mode=ParseMode.HTML)


async def cmd_lock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _fast_lock_unlock(update, context, enabled=False)


async def cmd_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _fast_lock_unlock(update, context, enabled=True)


async def _fast_lock_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE, enabled: bool):
    msg = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    if chat.type == "private" or user is None:
        return
    parts = (msg.text or "").split()
    if len(parts) != 2 or parts[1].lower() not in PERMISSION_FIELD_MAP:
        await msg.reply_text(
            stylize(f"ℹ️ <b>Use:</b> <code>/{'unlock' if enabled else 'lock'} &lt;type&gt;</code>\n"
            "Types: messages, photos, videos, audios, documents, stickers_gifs, polls, voice_notes, "
            "video_notes, link_previews, invite_users, pin_messages, change_info"),
            parse_mode=ParseMode.HTML,
        )
        return
    result = {"action": "toggle_permission", "permission": parts[1].lower(), "enabled": enabled, "confidence": "high", "reason": "Admin request"}
    await execute_admin_action(context, update, result, user, msg.text or "")
...
async def admins_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text(stylize("❌ <b>Admin list nahi la paya.</b>"), parse_mode=ParseMode.HTML)
        return
    owner_lines, admin_lines = [], []
    for m in members:
        u = m.user
        link = f"<a href='tg://user?id={u.id}'>{md_escape(u.first_name)}</a>"
        if m.status == ChatMemberStatus.OWNER:
            owner_lines.append(f"👑 {link}")
        else:
            admin_lines.append(f"🛡️ {link}")
    lines = ["📋 <b>GROUP OWNER & ADMINS</b> 📋\n"]
    lines += owner_lines
    lines += admin_lines
    lines.append(f"\nTotal admins: {len(admin_lines)} + 1 owner")
    final_text, final_entities = render_with_emojis("\n".join(lines))
    await context.bot.send_message(chat.id, final_text, entities=final_entities)
...
async def dm_welcome_media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """If the owner/granted-admin sends a photo/video/GIF (with caption)
    while in the welcome-text-edit flow, save it as the group's fixed
    welcome media -- many group owners don't want the new member's OWN
    profile photo shown, they want their own banner/logo instead."""
    if update.effective_chat.type != "private":
        return
    ...
    if caption_raw.strip():
        newly_saved = await asyncio.to_thread(
            auto_save_new_emojis_from_message, caption_raw, msg.caption_entities, user_id
        )
        await asyncio.to_thread(set_group_setting, chat_id, "welcome_text", caption_raw.strip())
    else:
        newly_saved = []
    await asyncio.to_thread(set_group_setting, chat_id, "welcome_media_file_id", file_id)
    await asyncio.to_thread(set_group_setting, chat_id, "welcome_media_type", media_type)

    context.user_data["panel_state"] = None
    reply = f"✅ <b>Welcome {media_type} set ho gaya!</b>" + (" Caption bhi text ke saath set ho gaya." if caption_raw.strip() else "")
    if newly_saved:
        reply += f"\n💾 <b>{len(newly_saved)} naye premium emoji bhi save ho gaye.</b>"
    await msg.reply_text(stylize(reply), parse_mode=ParseMode.HTML, reply_markup=group_panel_markup(chat_id))
...
async def dm_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == AWAITING_API_KEY and user_id == OWNER_ID:
        context.user_data["panel_state"] = None
        key = (update.effective_message.text or "").strip()
        if not key:
            return
        count = await asyncio.to_thread(add_ai_api_key, key, user_id)
        try:
            await update.effective_message.delete()  # don't leave the raw key sitting in chat history
        except TelegramError:
            pass
        await context.bot.send_message(
            user_id,
            stylize(f"✅ <b>API key saved!</b> Ab total {count} active key(s) hain, rotation mein use hongi."),
            parse_mode=ParseMode.HTML,
        )
        return
...
async def job_backup_to_channel(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Pin it so a fresh deploy (empty local DB) can auto-restore the
        # LATEST backup on startup via getChat().pinned_message -- bots
        # have no generic way to read a channel's older message history,
        # pinning is the only retroactively-queryable reference available.
        try:
            await context.bot.pin_chat_message(int(BACKUP_CHANNEL_ID), sent.message_id, disable_notification=True)
        except TelegramError:
            pass
    except Exception as e:
...
async def job_prune_admin_log(context: ContextTypes.DEFAULT_TYPE):
    """Weekly: drop admin_log rows older than 30 days to keep the DB/backup compact."""
...
async def job_ghost_account_cleanup(context: ContextTypes.DEFAULT_TYPE):
    ...
    Capped per run so a very large user table can't turn into an hours-long
    scan or a burst of API calls."""
    conn = db_connect()
    rows = conn.execute("SELECT DISTINCT user_id, chat_id FROM users LIMIT 300").fetchall()
    conn.close()
    removed = 0
    for row in rows:
        try:
...
async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Also let the person actually SEE that something broke, right in the
    # chat where it happened -- silent failures are how bugs like this go
    # unnoticed for a long time. Best-effort only; never let this itself
    # raise (e.g. no chat to reply into, or the bot got kicked meanwhile).
    try:
        if isinstance(update, Update) and update.effective_chat is not None:
            await context.bot.send_message(
                update.effective_chat.id,
                stylize("⚠️ <b>Kuch gadbad ho gayi is command mein — dobara try karo.</b> "
                "Agar baar-baar ho raha hai toh owner ko batao."),
                parse_mode=ParseMode.HTML,
            )
    except Exception:
        pass
    ...
    async def _call_upgraded(self, parent_method, text_field, entities_field, args, kwargs):
        try:
            ...
            if "not modified" in str(e).lower():
                return None  # harmless no-op -- desired content is already there
...
async def auto_restore_from_backup_channel(bot):
    """On a fresh deploy (Railway wipes local storage), the local DB starts
    completely empty -- this detects that and automatically pulls + imports
    the LATEST pinned backup from BACKUP_CHANNEL_ID, so the owner never has
    to manually export/import again after every redeploy. Only runs when
    the DB genuinely looks fresh (no approved groups AND no known groups
    at all) -- never touches a DB that already has real data, so a normal
    restart is completely unaffected."""
    if not BACKUP_CHANNEL_ID:
        return
    conn = db_connect()
    has_approved = conn.execute("SELECT 1 FROM approved_groups LIMIT 1").fetchone()
    has_known = conn.execute("SELECT 1 FROM known_groups LIMIT 1").fetchone()
    conn.close()
    if has_approved or has_known:
        return  # DB already has real data -- not a fresh wipe, don't touch it

    try:
        chat = await bot.get_chat(int(BACKUP_CHANNEL_ID))
        pinned = chat.pinned_message
        if pinned is None or pinned.document is None:
            logger.info("Auto-restore: no pinned backup found in BACKUP_CHANNEL_ID.")
            return
        file = await bot.get_file(pinned.document.file_id)
        raw_bytes = await file.download_as_bytearray()
        ok, message_text = db_import_json(bytes(raw_bytes).decode("utf-8"))
        if ok:
            logger.info("Auto-restore: latest backup restored successfully on startup.")
            try:
                await bot.send_message(OWNER_ID, "🔄 <b>Fresh deploy detected — latest backup auto-restored successfully.</b>", parse_mode=ParseMode.HTML)
            except TelegramError:
                pass
        else:
            logger.warning("Auto-restore: import failed -- %s", message_text)
    except Exception as e:
        logger.warning("Auto-restore from backup channel failed: %s", e)
...
def main():
    ...
    db_init()
    if not get_active_ai_api_keys():
        logger.warning("⚠️ No AI API keys set -- DM the bot /apisetup (owner only) to add one. AI features won't work until then.")
    _refresh_custom_emoji_cache()
    _refresh_approved_groups_cache()

    async def _post_init(application):
        # Mass-join floods fire MANY asyncio.to_thread() DB calls at once
        # (each welcome does several). The default executor is tiny on
        # small containers (min(32, cpu_count+4), often ~5 threads) and
        # was getting saturated, starving the admin ".." command's own
        # to_thread AI call behind a huge backlog -- this is what made the
        # bot look completely stuck during raids/mass joins even though
        # welcomes kept trickling through.
        loop = asyncio.get_running_loop()
        loop.set_default_executor(concurrent.futures.ThreadPoolExecutor(max_workers=64))

        await auto_restore_from_backup_channel(application.bot)
        # Caches were loaded above against the (possibly empty, pre-restore)
        # DB -- refresh again in case auto-restore just populated it.
        _refresh_custom_emoji_cache()
        _refresh_approved_groups_cache()

    builder = ApplicationBuilder().post_init(_post_init)
    try:
    ...
    app.add_handler(CommandHandler("welcomeimage", welcome_image_toggle_command))
    app.add_handler(CommandHandler("addblock", addblock_command))
    app.add_handler(CommandHandler("rmblock", rmblock_command))
    ...
    app.add_handler(CommandHandler("setabusemute", setabusemute_command))
    app.add_handler(CommandHandler("setlinkmute", setlinkmute_command))
    app.add_handler(CommandHandler("setabusewarn", setabusewarn_command))
    app.add_handler(CommandHandler("setlinkwarn", setlinkwarn_command))
    app.add_handler(CommandHandler("modsettings", modsettings_command))
    app.add_handler(CommandHandler("id", id_command))
    app.add_handler(CommandHandler("apisetup", apisetup_command))
    ...
    app.add_handler(CommandHandler("promote", cmd_admin))
    app.add_handler(CommandHandler("unadmin", cmd_unadmin))
    app.add_handler(CommandHandler("demote", cmd_unadmin))
    ...
    app.add_handler(CallbackQueryHandler(rules_button_callback, pattern="^show_rules$"))
    ...
    app.add_handler(MessageHandler((filters.PHOTO | filters.VIDEO | filters.ANIMATION) & filters.ChatType.PRIVATE, dm_welcome_media_handler))
    ...
    job_queue.run_repeating(job_prune_admin_log, interval=604800, first=3600)  # weekly, first run after 1hr
