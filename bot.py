...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.strip().endswith(".."):
        ...
        # Capture every reply_text call the actions make (instead of each
        # sending its own message) so it can all be combined into ONE
        # clean, decorated message at the end.
        captured = []
        real_reply_text = msg.reply_text

        async def _capture_reply(text_arg, *a, **kw):
            captured.append(text_arg)
            return None

        msg.reply_text = _capture_reply
        try:
            for single_action in actions_list:
                await execute_admin_action(context, update, single_action, user, command_text)
        finally:
            msg.reply_text = real_reply_text
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if action == "ban":
                ...
                if duration:
                    await msg.reply_text(stylize(f"🔇 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> {duration} min ke liye mute!</b>"), parse_mode=ParseMode.HTML)
                else:
                    await msg.reply_text(stylize(f"🔇 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> permanently mute kar diya gaya</b> (jab tak manually unmute na ho)!"), parse_mode=ParseMode.HTML)
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            await msg.reply_text(stylize("🔒 <b>Chat lock kar diya gaya — sirf admins bhej sakte hain.</b>"), parse_mode=ParseMode.HTML)
            ...
            await msg.reply_text(stylize("🔓 <b>Chat unlock kar diya gaya!</b>"), parse_mode=ParseMode.HTML)
            ...
            if not perm_key:
                await msg.reply_text(stylize("🤔 <b>Samajh nahi aaya kaunsi setting badalni hai.</b>"), parse_mode=ParseMode.HTML)
            else:
