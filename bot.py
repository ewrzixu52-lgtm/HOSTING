...
def stylize(html_text: str) -> str:
    ...
    for tok in tokens:
        if tok.startswith("<"):
            low = tok.lower()
            if low.startswith("<a ") or low.startswith("<code") or low.startswith("<tg-emoji"):
                skip_mode = True
            elif low.startswith("</a>") or low.startswith("</code>") or low.startswith("</tg-emoji>"):
                skip_mode = False
...
# --- custom / premium emoji auto-save system --------------------------------
# Lets an admin send a Telegram Custom/Premium Emoji straight to the bot
# (via /saveemoji) and have its custom_emoji_id auto-detected and stored,
# instead of typing the ID by hand. Saved emojis can then be re-used by
# name anywhere a filter reply or welcome-text template is rendered, using
# {name} -- see expand_placeholders_and_emojis() below.
def add_custom_emoji(chat_id, emoji_id, display_text, name, added_by):
    """Returns 'added', 'duplicate_emoji' (this exact emoji already saved in
    this chat, under whatever name it already has), or 'name_taken' (the
    requested name is already used by a different emoji in this chat)."""
    conn = db_connect()
    existing = conn.execute(
        "SELECT name FROM custom_emojis WHERE chat_id=? AND emoji_id=?", (chat_id, emoji_id)
    ).fetchone()
    if existing:
        conn.close()
        return "duplicate_emoji", existing["name"]
    name_clash = conn.execute(
        "SELECT 1 FROM custom_emojis WHERE chat_id=? AND name=?", (chat_id, name)
    ).fetchone()
    if name_clash:
        conn.close()
        return "name_taken", None
    conn.execute(
        "INSERT INTO custom_emojis (chat_id, emoji_id, display_text, name, added_by, timestamp) VALUES (?,?,?,?,?,?)",
        (chat_id, emoji_id, display_text, name, added_by, datetime.now(IST).isoformat()),
    )
    conn.commit()
    conn.close()
    return "added", None
...
def get_custom_emoji_by_name(chat_id, name):
    conn = db_connect()
    row = conn.execute(
        "SELECT emoji_id, display_text FROM custom_emojis WHERE chat_id=? AND name=?", (chat_id, name)
    ).fetchone()
    conn.close()
    return dict(row) if row else None


def list_custom_emojis(chat_id):
    conn = db_connect()
    rows = conn.execute(
        "SELECT name, emoji_id, display_text FROM custom_emojis WHERE chat_id=? ORDER BY name", (chat_id,)
    ).fetchall()
    conn.close()
    return rows


def rename_custom_emoji(chat_id, old_name, new_name):
    conn = db_connect()
    clash = conn.execute("SELECT 1 FROM custom_emojis WHERE chat_id=? AND name=?", (chat_id, new_name)).fetchone()
    if clash:
        conn.close()
        return "name_taken"
    cur = conn.execute("UPDATE custom_emojis SET name=? WHERE chat_id=? AND name=?", (new_name, chat_id, old_name))
    conn.commit()
    ok = cur.rowcount > 0
    conn.close()
...
def delete_custom_emoji(chat_id, name):
    conn = db_connect()
    cur = conn.execute("DELETE FROM custom_emojis WHERE chat_id=? AND name=?", (chat_id, name))
    conn.commit()
    deleted = cur.rowcount > 0
    conn.close()
    return deleted
...
def _next_auto_emoji_name(chat_id):
    """Auto-generated fallback name (emoji1, emoji2, ...) used when no name
    was given for a detected custom emoji."""
    conn = db_connect()
    n = conn.execute("SELECT COUNT(*) c FROM custom_emojis WHERE chat_id=?", (chat_id,)).fetchone()["c"]
    conn.close()
    candidate = n + 1
    while True:
        name = f"emoji{candidate}"
        conn = db_connect()
        clash = conn.execute("SELECT 1 FROM custom_emojis WHERE chat_id=? AND name=?", (chat_id, name)).fetchone()
        conn.close()
        if not clash:
            return name
        candidate += 1
...
def expand_placeholders_and_emojis(chat_id, template, **kwargs):
    """Safely replaces {word} tokens in a template string.
    - Known keys passed in kwargs (name/group/id/username, etc.) are filled in.
    - Any other {word} is checked against this chat's saved custom emojis
      (added via /saveemoji) and, if found, replaced with a <tg-emoji> HTML
      tag so the ORIGINAL premium/custom emoji renders (not a plain lookalike).
    - Anything that matches neither is left exactly as-is.
    Unlike str.format(), this NEVER raises on a stray/unknown '{...}' --
    that used to crash the entire welcome message the moment the saved
    welcome_text contained any brace str.format() didn't recognize.
    """
    if not template:
        return template
    ...
    def repl(m):
        ...
        emoji = get_custom_emoji_by_name(chat_id, key)
        if emoji:
            glyph = md_escape(emoji["display_text"] or "😀")
            return f'<tg-emoji emoji-id="{emoji["emoji_id"]}">{glyph}</tg-emoji>'
        return m.group(0)
...
async def ai_classify_admin_command(command_text: str, has_reply_target: bool) -> dict:
    """
    ...
    system_prompt = (
        ...
        "casual/desi andaaz me, gaali-galoch ya slang me bhi ho sakta hai, seedha "
        "command jaisa na lagkar normal insaan ki tarah bola ho sakta hai). Tumhara "
        "kaam sirf yeh samajhna hai ki woh kaunsa admin action maang raha hai. Tumhe "
        "koi exact keyword-matching nahi karni, poora sentence ka meaning aur intent "
        "samajhna hai, jaise ek insaan samajhta.\n\n"
        ...
        "COMMON DESI/CASUAL PHRASING PER ACTION (yeh sirf examples hain, aur bhi bahut "
        "tarike ho sakte hain -- inhi tak seemित mat raho, meaning samajhkar match karo):\n"
        "- kick: 'isko laat maaro', 'iski laat maar do', 'isko bhagao', 'isko group se "
        "nikal do', 'isko theek karo bahar ka rasta dikhao', 'kick maar isko', 'nikal "
        "baahar isko'.\n"
        "- ban: 'isko hamesha ke liye nikal do', 'isko permanent bhagao', 'iski entry "
        "band kar do', 'isko block kar do group se', 'dobara na aaye isliye nikal do', "
        "'uda do isko'.\n"
        "- mute: 'isko chup kara do', 'iski bakwas band karo', 'isko bolne mat do', "
        "'isko gag kar do', 'iski awaaz band kardo kuch der ke liye'.\n"
        "- warn: 'isko daant do', 'isko warning de do', 'isko ek chance aur do warning "
        "ke sath', 'thoda daata do isko'.\n"
        "- info: 'iski jaankari do', 'iska info batao', 'yeh kaun hai batao', 'iski "
        "detail do', 'iske baare me batao', 'iska data dikhao', 'kaun hai yeh banda'.\n"
        "- promote: 'isko admin bana do', 'isko power de do', 'isko admin panel do'.\n"
        "- demote: 'iska admin hata do', 'isko normal member bana do', 'iski power "
        "cheen lo'.\n"
        "- pin/unpin: 'isko upar chipka do' / 'pin hataa do', 'isko sabse upar rakho'.\n"
        "- delete: 'isko hata do', 'yeh message udaa do', 'ise mita do'.\n\n"
        "Rules:\n"
        ...
        "- 'delete' vs 'purge' me FARAK hai: agar sirf EK specific message delete karna hai "
        "(jaise 'yeh message delete karo', 'isko delete kar do', 'is message ko hata do') to "
        "action='delete' do. Agar REPLY se lekar is command tak ke SAARE messages ek saath "
        "delete/saaf karne hain (jaise 'yahan tak delete kar do', 'yahan tak purge karo', "
        "'is jagah se yahan tak sab clean kar do', 'in sab messages ko hata do') to "
        "action='purge' do.\n"
        "- 'set_title' matlab admin ka naam ke bagal me jo chota tag/title dikhta hai "
        "(jaise 'Admin', 'Manager') use badalna hai. Agar aisa kuch bola gaya ho (jaise "
        "'iska tag change karo Manager karke', 'naam badal do isko VIP', 'title set karo "
        "Support') to action='set_title' do aur naya title text 'title' field me nikaalo "
        "(sirf woh chota naam, quotes ya extra words ke bina, max 16 characters).\n"
        ...
        "- 'warn' ya 'unwarn' ke saath agar KITNI baar/kitni warnings ka zikar ho (jaise "
        "'2 warning hata do', '3 baar warn karo', 'iski saari warnings hata do') to woh "
        "number 'count' field me do. 'saari/sabhi/all warnings hata do' jaisa bola ho to "
        "count=999 do (backend isko 'jitni bhi hain sab' samjhega). Kuch na bola ho to "
        "count=1 (default, sirf ek).\n"
        "- 'upgrade_trust' ya 'downgrade_trust' ke saath agar KITNE level ek saath badhane/"
        "ghataane hain (jaise 'level 2 upgrade karo', '3 level badha do') to woh number "
        "'count' field me do. Agar 'max/maximum/top/sabse upar/full level kar do' jaisa "
        "bola ho to count=999 do (backend isko 'top level pe seedha' samjhega). Kuch na "
        "bola ho to count=1 (default, sirf ek level).\n"
        "- action='unknown' SIRF tab do jab sach me kuch bhi samajh na aaye ki kya "
        "maanga ja raha hai. Agar meaning approx bhi clear ho raha hai (chahe wording "
        "ajeeb ho, sarcastic ho, ya casual ho) to closest matching action do, unknown "
        "mat do -- confidence 'medium' ya 'low' de sakte ho par action zaroor do.\n"
        ...
        'Respond ONLY with compact JSON: {"action": "...", "duration_minutes": null, "title": null, "count": 1, "confidence": "high|medium|low"}'
    )
...
async def is_admin_fresh(bot, chat_id, user_id) -> bool:
    """Same check the natural-language 'filter karo (...)' flow already uses
    for a sensitive, rare action -- bypasses the 60s admin cache and asks
    Telegram directly, so it can never act on a stale cached result. Used by
    every custom-emoji save/rename/delete command (admin/owner only)."""
    if user_id == OWNER_ID:
        return True
    try:
        member = await bot.get_chat_member(chat_id, user_id)
...
async def handle_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        ...
        uname_line = f"🔗 <b>Username:</b> @{md_escape(member.username)}\n" if member.username else ""
        profile_link = user_profile_link(member.id, member.username)
        group_name = md_escape(chat.title or "is group")

        try:
            await _send_welcome_for_member(context, chat, member, name, uname_line, profile_link, group_name)
        except Exception as e:
            # Defense in depth: no matter what goes wrong building/sending the
            # welcome message for THIS member, it must never stop the rest of
            # this handler (other members in the same join event, the raid
            # tracking above, or the impersonation check) from running.
            logger.warning("Welcome message build/send failed for member %s in chat %s: %s", member.id, chat.id, e)
...
async def _send_welcome_for_member(context, chat, member, name, uname_line, profile_link, group_name):
    gsettings = await asyncio.to_thread(get_group_settings, chat.id)

    if gsettings.get("welcome_text"):
        # Custom per-group welcome text -- supports {name}/{group}/{id}/{username}
        # placeholders, PLUS any {emoji_name} saved via /saveemoji. Uses
        # expand_placeholders_and_emojis() instead of str.format() on purpose:
        # str.format() throws a KeyError/crashes the ENTIRE welcome message the
        # instant the saved text contains any "{...}" it doesn't recognize
        # (e.g. someone typing "{fire}" before that emoji was actually saved,
        # or any stray brace) -- that was silently killing welcome messages for
        # everyone in the group, not just showing a broken placeholder. This
        # version never raises: an unrecognized "{word}" is just left as-is.
        caption = await asyncio.to_thread(
            expand_placeholders_and_emojis,
            chat.id, gsettings["welcome_text"],
            name=name, group=group_name, id=member.id,
            username=(f"@{md_escape(member.username)}" if member.username else "N/A"),
        )
    else:
        caption = (
            f"🎉 <b>WELCOME to {group_name}, {name}!</b> 🎉\n\n"
            f"🆔 <b>User ID:</b> <code>{member.id}</code>\n"
            f"{uname_line}"
            f"👤 <b>Profile:</b> <a href='{profile_link}'>Click Here</a>\n\n"
            f"✅ <b>SELLING ALLOWED, BUT ALWAYS USE ESCROW FOR ANY DEAL.</b>\n"
            f"🚫 <b>PROMOTION NOT ALLOWED.</b>\n\n"
            f"💬 Chat karo, enjoy karo 🎉"
        )

    buttons = []
    if gsettings.get("channel_username"):
        buttons.append([InlineKeyboardButton(f"📢 {gsettings['channel_username']}", url=f"https://t.me/{gsettings['channel_username']}", api_kwargs={"style": "success"})])
    else:
        for ch_username in get_welcome_channels():
            buttons.append([InlineKeyboardButton(f"📢 {ch_username}", url=f"https://t.me/{ch_username}", api_kwargs={"style": "success"})])

    if gsettings.get("button_text") and gsettings.get("button_url"):
        buttons.append([InlineKeyboardButton(fancy(gsettings["button_text"]), url=gsettings["button_url"], api_kwargs={"style": "primary"})])
    else:
        buttons.append([InlineKeyboardButton(fancy("💰 Escrow: Contact Owner"), url=f"https://t.me/{OWNER_USERNAME}", api_kwargs={"style": "primary"})])
    ...
    try:
        if photo_bytes:
            await context.bot.send_photo(
                chat.id, photo=bytes(photo_bytes), caption=stylize(caption),
                parse_mode=ParseMode.HTML, reply_markup=markup,
            )
        else:
            await context.bot.send_message(
                chat.id, stylize(caption), parse_mode=ParseMode.HTML,
...
async def send_filter_reply(context, chat_id, matched_filter, reply_to_message_id):
    try:
        ...
        # Expand any {name} tokens against this chat's saved custom emojis
        # (/saveemoji) BEFORE stylizing/sending, so a filter reply like
        # "welcome {fire}" sends the actual saved premium emoji.
        raw_reply = matched_filter["reply_text"] or ""
        reply_html = stylize(expand_placeholders_and_emojis(chat_id, raw_reply)) if raw_reply else None
        if ftype == "photo":
            await context.bot.send_photo(chat_id, matched_filter["file_id"], caption=reply_html, parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
        elif ftype == "sticker":
            await context.bot.send_sticker(chat_id, matched_filter["file_id"], reply_to_message_id=reply_to_message_id)
        elif ftype == "animation":
            await context.bot.send_animation(chat_id, matched_filter["file_id"], caption=reply_html, parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
        elif ftype == "video":
            await context.bot.send_video(chat_id, matched_filter["file_id"], caption=reply_html, parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
        elif ftype == "document":
            await context.bot.send_document(chat_id, matched_filter["file_id"], caption=reply_html, parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
        elif reply_html:
            await context.bot.send_message(chat_id, reply_html, parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
    except TelegramError:
        pass
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if filter_match:
        ...
        if trigger:
            if is_remove:
                ...
                # IMPORTANT: use .text_html/.caption_html (not .text/.caption) so
                # bold/italic AND custom/premium emoji survive. Plain .text only
                # gives the placeholder character behind a premium emoji, with no
                # way to know it was a premium emoji at all -- that's why premium
                # emoji used to show as a normal emoji once the filter fired.
                # .text_html/.caption_html encode that as a <tg-emoji emoji-id=...>
                # tag, which Telegram renders as the ORIGINAL premium emoji again
                # when the reply is sent back out with parse_mode=HTML.
                reply_text_val = rmsg.text_html or rmsg.caption_html or rmsg.text or rmsg.caption or ""
        ...
        if fake_mention:
            ...
            if result.get("is_impersonation"):
                ...
                markup = InlineKeyboardMarkup(
                    [[InlineKeyboardButton(fancy("💰 Contact Owner for Escrow"), url=f"https://t.me/{OWNER_USERNAME}", api_kwargs={"style": "primary"})]]
                )
                await context.bot.send_message(
                    chat.id,
                    stylize("⚠️ <b>Kisi aur escrow worker ke paas mat jaayein.</b>\n"
                    "✅ <b>Sirf owner se contact karein.</b> 👇"),
                    parse_mode=ParseMode.HTML,
                    reply_markup=markup,
                )
                return
    ...
    if admin_ok and msg.reply_to_message is not None and "info" in _info_words and len(_info_words) <= 4:
        ...
        if t is not None and not t.is_bot:
            ...
            info_text = (
                ...
                f"<b>Username:</b> @{t.username if t.username else 'N/A'}\n"
            ...
            try:
                await msg.reply_text(stylize(info_text), parse_mode=ParseMode.HTML)
            except TelegramError:
                pass
            return
    ...
    if text.strip().endswith(".."):
        ...
        result = await ai_classify_admin_command(command_text, has_reply)
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if action == "ban":
                ...
                if target.id == OWNER_ID:
                    ...
                    new_title = str(title_text)[:16]  # Telegram's own 16-character limit
                    try:
...
async def send_user_info(context, chat_id, target):
    ...
    await context.bot.send_message(chat_id, stylize(text), parse_mode=ParseMode.HTML)
...
# CUSTOM / PREMIUM EMOJI AUTO-SAVE SYSTEM
# =============================================================================
# /saveemoji [name1 name2 ...]  -- ADMIN/OWNER ONLY.
#   Send this command in the group with one or more Custom/Premium emoji
#   inside the SAME message (just pick them from the emoji keyboard like any
#   other emoji -- no manual ID typing). The bot reads the message's own
#   entities, pulls out every custom_emoji_id automatically, and saves each
#   one. If you give names ("/saveemoji fire ice") they're assigned in the
#   order the emojis appear in the message; any emoji left without a name
#   gets an auto name (emoji1, emoji2, ...) that you can fix with /renameemoji.
async def saveemoji_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    if chat.type == "private" or user is None:
        await msg.reply_text(stylize("ℹ️ <b>Yeh command group me use karo, jahan emoji save karna hai.</b>"), parse_mode=ParseMode.HTML)
        return

    if not await is_admin_fresh(context.bot, chat.id, user.id):
        return  # silently ignore, same as the filter-set guard

    entities = msg.parse_entities(types=["custom_emoji"]) if msg.entities else {}
    if not entities and msg.caption_entities:
        entities = msg.parse_caption_entities(types=["custom_emoji"])
    found = [(ent, txt) for ent, txt in sorted(entities.items(), key=lambda kv: kv[0].offset)]

    if not found:
        await msg.reply_text(
            stylize("❌ <b>Isme koi Custom/Premium Emoji nahi mila.</b>\n"
                    "Emoji keyboard se premium emoji pick karke <code>/saveemoji naam</code> ke saath bhejo."),
            parse_mode=ParseMode.HTML,
        )
        return

    given_names = context.args or []
    result_lines = ["✅ <b>Custom Emoji Saved</b>\n"]
    for i, (ent, display_text) in enumerate(found):
        name = given_names[i].lower() if i < len(given_names) else await asyncio.to_thread(_next_auto_emoji_name, chat.id)
        status, extra = await asyncio.to_thread(add_custom_emoji, chat.id, ent.custom_emoji_id, display_text, name, user.id)
        if status == "duplicate_emoji":
            result_lines.append(f"ℹ️ Yeh emoji pehle se <code>{md_escape(extra)}</code> naam se saved hai.")
        elif status == "name_taken":
            result_lines.append(f"⚠️ Naam <code>{md_escape(name)}</code> already kisi aur emoji ke liye use ho raha hai — is emoji ko rename karke dobara try karo.")
        else:
            result_lines.append(
                f"Name: <code>{md_escape(name)}</code>\n"
                f"ID: <code>{md_escape(ent.custom_emoji_id)}</code>\n"
                f'<tg-emoji emoji-id="{ent.custom_emoji_id}">{md_escape(display_text)}</tg-emoji>  '
                f"→ ab isse <code>{{{md_escape(name)}}}</code> likh kar filter/welcome text me use kar sakte ho."
            )
    await msg.reply_text("\n\n".join(result_lines), parse_mode=ParseMode.HTML)


async def emojis_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone can VIEW the saved list (same visibility as /filters) -- only
    saving/renaming/deleting is admin-restricted."""
    chat = update.effective_chat
    if chat.type == "private":
        return
    rows = await asyncio.to_thread(list_custom_emojis, chat.id)
    if not rows:
        await update.effective_message.reply_text(stylize("😶 <b>Is group me koi custom emoji save nahi hai.</b>"), parse_mode=ParseMode.HTML)
        return
    lines = ["🎨 <b>SAVED CUSTOM EMOJIS</b> 🎨\n"]
    for r in rows:
        lines.append(f'• <code>{{{md_escape(r["name"])}}}</code> → <tg-emoji emoji-id="{r["emoji_id"]}">{md_escape(r["display_text"] or "😀")}</tg-emoji>')
    await update.effective_message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)


async def renameemoji_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    if chat.type == "private" or user is None:
    ...
    if len(context.args) != 2:
        await msg.reply_text(stylize("ℹ️ <b>Use:</b> <code>/renameemoji purana_naam naya_naam</code>"), parse_mode=ParseMode.HTML)
        return
    old_name, new_name = context.args[0].lower(), context.args[1].lower()
    result = await asyncio.to_thread(rename_custom_emoji, chat.id, old_name, new_name)
    if result == "ok":
        await msg.reply_text(stylize(f"✅ <b>Rename ho gaya:</b> <code>{{{md_escape(old_name)}}}</code> → <code>{{{md_escape(new_name)}}}</code>"), parse_mode=ParseMode.HTML)
    elif result == "name_taken":
        await msg.reply_text(stylize(f"⚠️ Naam <code>{md_escape(new_name)}</code> already use ho raha hai."), parse_mode=ParseMode.HTML)
    else:
        await msg.reply_text(stylize(f"❌ <code>{md_escape(old_name)}</code> naam ka koi saved emoji nahi mila."), parse_mode=ParseMode.HTML)


async def delemoji_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    if chat.type == "private" or user is None:
    ...
    if not await is_admin_fresh(context.bot, chat.id, user.id):
        return
    if not context.args:
        await msg.reply_text(stylize("ℹ️ <b>Use:</b> <code>/delemoji naam</code>"), parse_mode=ParseMode.HTML)
        return
    name = context.args[0].lower()
    deleted = await asyncio.to_thread(delete_custom_emoji, chat.id, name)
    await msg.reply_text(
        stylize(f"✅ <b>Deleted:</b> <code>{{{md_escape(name)}}}</code>" if deleted else f"❌ <code>{md_escape(name)}</code> naam ka koi saved emoji nahi mila."),
        parse_mode=ParseMode.HTML,
    )
...
async def grant_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        stylize(f"✅ <b>Access de diya gaya!</b>\nUser <code>{target_user_id}</code> ab "
...
async def revoke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner-only, DM: /revoke <user_id> <chat_id>"""
    if update.effective_chat.type != "private" or update.effective_user.id != OWNER_ID:
        return
    parts = (update.effective_message.text or "").split()
    if len(parts) != 3:
        await update.effective_message.reply_text(stylize("ℹ️ <b>Use:</b> <code>/revoke &lt;user_id&gt; &lt;group_id&gt;</code>"), parse_mode=ParseMode.HTML)
        return
    try:
...
def group_panel_markup(chat_id):
    buttons = [
        [InlineKeyboardButton(fancy("✏️ Welcome Message"), callback_data=f"gpanel_wtext_{chat_id}", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton(fancy("🔘 Button (text + link)"), callback_data=f"gpanel_button_{chat_id}", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton(fancy("📢 Channel Link"), callback_data=f"gpanel_channel_{chat_id}", api_kwargs={"style": "success"})],
    ]
...
async def group_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "wtext":
        ...
        await query.edit_message_text(
            stylize("✏️ <b>Naya welcome message bhejo.</b>\n\n"
            "Placeholders use kar sakte ho: <code>{name}</code>, <code>{group}</code>, "
            "<code>{id}</code>, <code>{username}</code>\n\n"
            "Example: <code>Welcome to {group}, {name}!</code>"),
            parse_mode=ParseMode.HTML,
        )
    elif field == "button":
...
def admin_panel_markup():
    buttons = [
        [InlineKeyboardButton(fancy("📢 Manage Welcome Channels"), callback_data="panel_channels", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton(fancy("📤 Export Database"), callback_data="panel_export", api_kwargs={"style": "success"}),
         InlineKeyboardButton(fancy("📥 Import Database"), callback_data="panel_import", api_kwargs={"style": "success"})],
        [InlineKeyboardButton(fancy("🏆 Trusted Traders"), callback_data="panel_trusted", api_kwargs={"style": "primary"})],
    ]
    return InlineKeyboardMarkup(buttons)
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_channels":
        ...
        buttons = [[InlineKeyboardButton(f"{fancy('❌ Remove')} @{c}", callback_data=f"rmchan_{c}", api_kwargs={"style": "danger"})] for c in channels]
        buttons.append([InlineKeyboardButton(fancy("⬅️ Back"), callback_data="panel_back", api_kwargs={"style": "primary"})])
...
def main():
    ...
    app = (
        ...
        # Explicit timeouts so a single flaky network call can never hang
        # forever -- it fails fast (raises TimedOut, auto-retried by PTB)
        # instead of blocking that update indefinitely.
        .connect_timeout(15)
        .read_timeout(20)
        .write_timeout(20)
        .pool_timeout(15)
    ...
    app.add_handler(CommandHandler("saveemoji", saveemoji_command))
    app.add_handler(CommandHandler("emojis", emojis_list_command))
    app.add_handler(CommandHandler("renameemoji", renameemoji_command))
    app.add_handler(CommandHandler("delemoji", delemoji_command))
