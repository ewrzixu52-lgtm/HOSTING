...
async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    ...
    system_prompt = (
        ...
        "make the whole group laugh out loud.\n\n"
        ...
        "roast must be instantly, clearly understandable on first read -- never "
        "confusing, never word-salad, never so abstract that nobody gets the "
        "joke.\n\n"
        "EXACT LEVEL/STYLE WANTED (write something THIS creative and THIS "
        "clear every time -- never reuse this specific one, invent a new "
        ...
        "2. Language: Hinglish ONLY -- Hindi ki tarah sunna chahiye par likha "
        "Roman/English script mein ho, bilkul jaisa log WhatsApp pe chat karte "
        "hain (Devanagari script kabhi mat use karna).\n"
        ...
        "that.\n"
        "5. Length: 1-2 sentences max, punchy. No explaining the joke afterward.\n"
        ...
        "roasting them for. Use their first name naturally. You may use 1 plain/"
        "normal emoji (not custom or made-up ones) at the very end if it fits."
    )
...
async def send_welcome_message(context: ContextTypes.DEFAULT_TYPE, chat, member):
    ...
    if gsettings.get("rules_text"):
        buttons.append([InlineKeyboardButton(fancy("📜 Rules"), callback_data="show_rules", style="success")])
...
async def rules_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rules:
        ...
        return
    try:
        final_text, final_entities = render_with_emojis(f"📜 <b>{md_escape(chat.title or 'Group')} Rules</b> 📜\n\n{rules}")
        await context.bot.send_message(query.from_user.id, final_text, entities=final_entities)
        # A real modal popup (show_alert=True), not the small fading toast --
        # and Telegram's callback-answer text is plain-string only (no HTML,
        # no entities), so no bold/premium-emoji is possible here no matter
        # what -- a normal emoji is the most this surface can ever show.
        await query.answer("📜 Rules padh lo achhe se!", show_alert=True)
    except TelegramError:
        await query.answer("⚠️ Pehle bot ko DM me /start karo, phir rules dekh paoge.", show_alert=True)
