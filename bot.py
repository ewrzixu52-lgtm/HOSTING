                ...
                try:
                    ...
                    for mid in batch:
                        try:
                            ...
                            # Sender (user, bot, or channel) makes no difference
                            # to deletion rights here -- log the real reason
                            # instead of silently dropping it, and retry once
                            # after a short delay in case it was a transient
                            # flood-control/propagation hiccup.
                            failed_ids.append((mid, str(e2)))
                            continue
            if failed_ids:
                await asyncio.sleep(1)
                still_failed = []
                for mid, _ in failed_ids:
                    try:
                        await context.bot.delete_message(chat.id, mid)
                        deleted += 1
                    except TelegramError as e3:
                        still_failed.append((mid, str(e3)))
                if still_failed:
                    logger.warning("Purge: %d message(s) could not be deleted after retry: %s", len(still_failed), still_failed)
