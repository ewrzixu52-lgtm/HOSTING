...
def render_with_emojis(html_text: str) -> tuple:
    """Converts our small HTML subset (<b>, <code>, <a href>) PLUS automatic
    premium-emoji substitution ({name} tokens AND any plain emoji that has a
    saved premium alternate) into (plain_text, entities) for sending WITHOUT
    parse_mode. This is the ONLY reliable way to make real custom/premium
    emoji show up in a bot-sent message -- the <tg-emoji> HTML tag was tried
    first but confirmed (via a real device screenshot) to render as literal
    text instead of an emoji, so real MessageEntity(CUSTOM_EMOJI) objects
    are used here instead, exactly like Telegram's own clients do it."""
    if not html_text:
        return "", []
...
# Telegram's fixed, permanent pseudo-account ID used as `from_user` whenever
# a message is posted "anonymously as the group" (admin has "Remain
# Anonymous" on, or hidden-members mode). It is NOT a real person -- looking
# up "info" on it was the bug that returned this fake account's ID instead
# of the actual group's ID.
...
def db_import_json(raw: str) -> tuple[bool, str]:
    ...
    try:
        conn = db_connect()
        cur = conn.cursor()
        for t in required_tables + [t for t in ("group_access", "group_settings", "filters", "custom_emojis", "blocked_groups") if t in data]:
        ...
        conn.commit()
        conn.close()
        # CRITICAL: the in-memory custom-emoji cache (used by stylize() /
        # render_with_emojis() on every single message) is only ever loaded
        # from DB at process startup. An import happens WHILE the process is
        # already running, so without this explicit refresh here, every
        # previously-saved premium emoji from the imported backup would sit
        # correctly in the DB but never actually render -- the bot would keep
        # using its stale (often empty, right after a Railway redeploy)
        # in-memory cache until the next full restart. This was the exact
        # bug: export -> redeploy -> import looked successful but all custom
        # emoji appeared to have "reset" to normal.
        _refresh_custom_emoji_cache()
        return True, "✅ <b>Database successfully import ho gaya! (Emoji cache bhi turant refresh ho gaya)</b>"
...
# --- group-level block/approve (fully disables the bot in a group) --------
def block_group(chat_id):
    """Owner revoked this group. Bot must stop doing ANYTHING here --
    moderation, welcome messages, filters, admin commands, everything --
    until the owner explicitly approves it again via /grant. Also strips
    every granted user's DM-panel access to this chat, since panel access
    to a fully-blocked group makes no sense."""
    conn = db_connect()
    conn.execute(
        "INSERT OR REPLACE INTO blocked_groups (chat_id, blocked_at) VALUES (?,?)",
        (chat_id, datetime.now(IST).isoformat()),
    )
    conn.execute("DELETE FROM group_access WHERE chat_id=?", (chat_id,))
    conn.commit()
    conn.close()


def unblock_group(chat_id):
    conn = db_connect()
    conn.execute("DELETE FROM blocked_groups WHERE chat_id=?", (chat_id,))
    conn.commit()
    conn.close()


def is_group_blocked(chat_id) -> bool:
    conn = db_connect()
    row = conn.execute("SELECT 1 FROM blocked_groups WHERE chat_id=?", (chat_id,)).fetchone()
    conn.close()
    return bool(row)
...
# =============================================================================
# GROUP BLOCK ENFORCEMENT -- when the owner has /revoke'd a group, the bot
# must stop doing ANYTHING there. Rate-limited notice (not sent on every
# single message, only when someone actually tries to USE the bot) so a
# blocked group doesn't get spammed.
# =============================================================================
_blocked_notice_last_sent = {}   # chat_id -> last-notified timestamp
_BLOCKED_NOTICE_COOLDOWN = 120   # seconds


async def notify_group_blocked_if_needed(context: ContextTypes.DEFAULT_TYPE, chat_id):
    now = time.time()
    last = _blocked_notice_last_sent.get(chat_id, 0)
    if (now - last) < _BLOCKED_NOTICE_COOLDOWN:
        return
    _blocked_notice_last_sent[chat_id] = now
    try:
        await context.bot.send_message(
            chat_id,
            stylize(f"🚫 <b>Is group ke liye bot approve nahi hai.</b>\n"
            f"Approval ke liye owner (@{OWNER_USERNAME}) se permission lein."),
...
# =============================================================================
_recently_welcomed = {}          # (chat_id, user_id) -> timestamp
_WELCOME_DEDUP_WINDOW = 20        # seconds


async def maybe_send_welcome_once(context: ContextTypes.DEFAULT_TYPE, chat, member):
    """Guards against sending the welcome message twice for the same join --
    needed because a join can now be detected via TWO independent Telegram
    update types (see handle_chat_member_update's docstring for why), and
    both could fire for the same real join."""
    key = (chat.id, member.id)
    now = time.time()
    last = _recently_welcomed.get(key)
    if last and (now - last) < _WELCOME_DEDUP_WINDOW:
        return
...
async def send_welcome_message(context: ContextTypes.DEFAULT_TYPE, chat, member):
    """Builds and sends the welcome message for one member. Shared by both
    the normal new_chat_members flow AND the chat_join_request flow (large/
    public groups with 'Approve New Members' enabled route joins through
    join-requests instead of a direct new_chat_members event)."""
...
async def handle_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if member.is_bot:
        return
    if await asyncio.to_thread(is_group_blocked, chat.id):
        return  # bot fully disabled in this group -- don't approve/welcome
    try:
        ...
        return
    await maybe_send_welcome_once(context, chat, member)


async def handle_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if await asyncio.to_thread(is_group_blocked, chat.id):
        return  # bot fully disabled in this group -- no welcome, no anti-raid
    now = time.time()
...
async def handle_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Catches joins via the chat_member update type -- this fires even when
    the group has 'Hide Members' turned ON, in which case Telegram does NOT
    send the normal 'X joined the group' service message (the event
    handle_new_members listens for) but STILL sends this chat_member status
    update. This is exactly how bots like Rose keep welcoming people even
    with Hide Members on -- they don't rely on the service message at all.

    Requires the bot to be an admin with the 'can_manage_chat' style access
    that lets it see chat_member updates (any group-admin bot already has
    this) and Update.ALL_TYPES to be passed to run_polling (already is).

    maybe_send_welcome_once() de-dupes against handle_new_members, so groups
    WITHOUT Hide Members (where both events can fire for the same join)
    never get double-welcomed."""
    ...
    was_in_chat = old_status in (ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER, ChatMemberStatus.RESTRICTED)
    now_in_chat = new_status in (ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER)
    if was_in_chat or not now_in_chat:
        return  # not a fresh join (could be a promotion/demotion/restriction change)
...
async def handle_unknown_group_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat = update.effective_chat
    if await asyncio.to_thread(is_group_blocked, chat.id):
        await notify_group_blocked_if_needed(context, chat.id)
        return
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------------------------------------------------------
    # -1.5 GROUP BLOCKED BY OWNER (/revoke <group_id>) -- bot must not do
    #      ANYTHING here (moderation, filters, admin commands, AI). Only
    #      send the "not approved" notice when someone actually looks like
    #      they're trying to USE the bot (admin ".." command), otherwise
    #      stay completely silent so normal chatting in the group is
    #      untouched by bot spam.
    # ---------------------------------------------------------------
    if await asyncio.to_thread(is_group_blocked, chat.id):
        raw_text_check = (msg.text or msg.caption or "").strip()
        if raw_text_check.endswith("..") or raw_text_check.lower().rstrip(".!?") == "info":
            await notify_group_blocked_if_needed(context, chat.id)
        return
    ...
    if admin_ok and msg.reply_to_message is not None and "info" in _info_words and len(_info_words) <= 4:
        ...
        if replied.sender_chat is not None or (replied.from_user and replied.from_user.id == GROUP_ANONYMOUS_BOT_ID):
            # Anonymous-admin / channel post reply -- there's no real user
            # here. Show the GROUP's own ID/info instead of Telegram's fake
            # "GroupAnonymousBot" placeholder account.
            await send_chat_info(context, chat.id, replied.sender_chat or chat)
            return
        t = replied.from_user
...
async def send_chat_info(context, reply_chat_id, target_chat):
    """Shows info about a GROUP/CHANNEL itself (not a user) -- used when the
    replied-to message was posted anonymously as the group, or is a linked
    channel post, so there's no real user to show info for."""
    chat_title = md_escape(getattr(target_chat, "title", None) or "N/A")
    chat_id_val = getattr(target_chat, "id", None)
    chat_username = getattr(target_chat, "username", None)
    text = (
        f"👥 <b>Group/Channel Info</b>\n\n"
        f"📛 Name: {chat_title}\n"
        f"🆔 Group ID: <code>{chat_id_val}</code>\n"
        f"🔗 Username: @{md_escape(chat_username) if chat_username else 'N/A'}\n\n"
        f"ℹ️ <i>Yeh message anonymously (group ki taraf se) bheja gaya tha, isliye koi individual user info nahi hai.</i>"
    )
    final_text, final_entities = render_with_emojis(text)
    await context.bot.send_message(reply_chat_id, final_text, entities=final_entities)
...
async def user_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = update.effective_message
    chat = update.effective_chat
    if chat.type != "private" and await asyncio.to_thread(is_group_blocked, chat.id):
        await notify_group_blocked_if_needed(context, chat.id)
        return
    if not msg.reply_to_message:
    ...
    replied = msg.reply_to_message
    if replied.sender_chat is not None or (replied.from_user and replied.from_user.id == GROUP_ANONYMOUS_BOT_ID):
        # Replied message was posted anonymously "as the group" (or is a
        # linked-channel post) -- there is no real user here, so show the
        # GROUP's own info instead of Telegram's fake "GroupAnonymousBot"
        # placeholder account (that placeholder is NOT the group, and was
        # being shown by mistake before this fix).
        await send_chat_info(context, chat.id, replied.sender_chat or chat)
        return
    await send_user_info(context, chat.id, replied.from_user)
...
async def grant_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Granting access = owner approving this group. If it was previously
    # revoked/blocked (bot fully disabled there), re-enable it now.
    await asyncio.to_thread(unblock_group, target_chat_id)
    try:
...
async def revoke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner-only, DM.
    /revoke <group_id>            -> FULLY blocks the bot in that group (real
                                      revoke -- bot stops working there completely,
                                      until /grant is used again for that group).
    /revoke <user_id> <group_id>  -> old narrower behaviour: only removes that
                                      one user's DM-panel customization access,
                                      does NOT block the bot in the group."""
    if update.effective_chat.type != "private" or update.effective_user.id != OWNER_ID:
        return
    parts = (update.effective_message.text or "").split()
    ...
    if len(parts) == 2:
        try:
            ...
            return
        await asyncio.to_thread(block_group, target_chat_id)
        await update.effective_message.reply_text(
            stylize(f"🚫 <b>Group <code>{target_chat_id}</code> ke liye bot completely block kar diya gaya.</b>\n"
            "Ab us group me bot koi bhi command/moderation/welcome message kuch nahi karega. "
            f"Dobara enable karne ke liye <code>/grant &lt;user_id&gt; {target_chat_id}</code> use karo."),
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    if len(parts) == 3:
        ...
        await update.effective_message.reply_text(
            stylize(f"✅ <b>User <code>{target_user_id}</code> ka panel access <code>{target_chat_id}</code> ke liye revoke kar diya gaya.</b>\n"
            "(Note: yeh sirf DM-panel access hai — bot us group me moderation ke liye kaam karta rahega. "
            "Poore group ko block karne ke liye <code>/revoke &lt;group_id&gt;</code> use karo.)"),
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    await update.effective_message.reply_text(
        stylize("ℹ️ <b>Use:</b>\n"
        "<code>/revoke &lt;group_id&gt;</code> — poore group ke liye bot completely block\n"
        "<code>/revoke &lt;user_id&gt; &lt;group_id&gt;</code> — sirf ek user ka panel access hatao"),
        parse_mode=ParseMode.HTML,
    )
...
def admin_panel_markup():
    buttons = [
        [InlineKeyboardButton(fancy("⚙️ My Groups (Welcome/Buttons)"), callback_data="panel_mygroups", api_kwargs={"style": "primary"})],
        [InlineKeyboardButton(fancy("💾 Save Custom Emoji"), callback_data="panel_saveemoji", api_kwargs={"style": "success"})],
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_mygroups":
        ...
        if not groups:
            ...
            return
        buttons = [[InlineKeyboardButton(fancy(g["title"] or str(g["chat_id"]))[:60], callback_data=f"gpick_{g['chat_id']}", api_kwargs={"style": "primary"})] for g in groups]
...
def main():
    ...
    # Catches joins even when the group has "Hide Members" on (Telegram then
    # skips the normal new_chat_members service message entirely) -- see
    # handle_chat_member_update's docstring.
