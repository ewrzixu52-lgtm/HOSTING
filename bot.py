...
LINK_PATTERN = re.compile(
    ...
    r"|(\bhttp\b)"                          # bare word "http"/"https" typed without ://
    ...
    r"|(@\s*(?:the\s*)?rate)"               # "at the rate" / "@ the rate" spelled-out @ symbol
    r"|(@[A-Za-z][A-Za-z0-9_]{3,})",         # @username mentions
    re.IGNORECASE,
)
# NOTE: bare "@username" is intentionally included here (re-added on the
# owner's request). This *will* fire the AI link-permission check more
# often for normal @tagging between members -- that's expected; the AI
# step (ai_check_link_permission) is what filters out genuine false
# positives (two members tagging each other) from real promotion/links,
# using the "was_requested_by_admin" + recent-context check, instead of
# just trusting the local regex blindly.
...
async def apply_warn_and_maybe_ban(context, chat_id, user_id, reason):
    ...
    count = add_warn(user_id, chat_id, reason)
    if count >= WARN_BAN_LIMIT:
        try:
            await context.bot.ban_chat_member(chat_id, user_id)
            record_ban(user_id, chat_id, f"Auto-banned: {WARN_BAN_LIMIT} warnings reached")
        except TelegramError as e:
