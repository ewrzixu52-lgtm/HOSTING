...
async def handle_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        ...
        uname_line = f"🔗 **Username:** @{member.username}\n" if member.username else ""
...
async def check_impersonation(context: ContextTypes.DEFAULT_TYPE, chat_id, member, photo_bytes):
    ...
    if overall >= 50:
        ...
        try:
            await context.bot.send_message(
                chat_id,
                f"⚠️ [{member.first_name}](tg://user?id={member.id}) **ka profile owner se "
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(dq) >= ANTIFLOOD_MAX_MSGS:
        ...
        try:
            await context.bot.send_message(
                chat.id,
                f"🚫 [{user.first_name}](tg://user?id={user.id}) **spam kar raha tha — "
    ...
    if contains_abuse(text):
        ...
        try:
            await context.bot.send_message(
                chat.id,
                f"🤬 [{user.first_name}](tg://user?id={user.id}) **ne gaali di — "
    ...
    if message_has_link_pattern(text):
        ...
        if result.get("is_link_or_promo") and not result.get("was_requested_by_admin"):
            ...
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🚫 [{user.first_name}](tg://user?id={user.id}), **promotion mat karo "
    ...
    if text.strip().endswith(".."):
        ...
        if not admin_ok:
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🔒 [{user.first_name}](tg://user?id={user.id}), **aap admin nahi ho.**",
                    parse_mode=ParseMode.MARKDOWN,
                )
            except TelegramError:
                pass
    ...
    if message_has_link_pattern(text):
        ...
        if result.get("is_link_or_promo") and not result.get("was_requested_by_admin"):
            ...
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🚫 [{user.first_name}](tg://user?id={user.id}), **message edit karke "
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if action == "ban":
                ...
                await msg.reply_text(f"🔨 **[{target.first_name}](tg://user?id={target.id}) ban kar diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "unban":
                await context.bot.unban_chat_member(chat.id, target.id, only_if_banned=True)
                remove_ban_record(target.id, chat.id)
                await msg.reply_text(f"✅ **[{target.first_name}](tg://user?id={target.id}) unban kar diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "kick":
                await context.bot.ban_chat_member(chat.id, target.id)
                await context.bot.unban_chat_member(chat.id, target.id)
                await msg.reply_text(f"👢 **[{target.first_name}](tg://user?id={target.id}) kick kar diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "mute":
                mins = duration or 60
                await mute_user(context, chat.id, target.id, mins)
                await msg.reply_text(f"🔇 **[{target.first_name}](tg://user?id={target.id}) {mins} min ke liye mute!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "unmute":
                await unmute_user(context, chat.id, target.id)
                await msg.reply_text(f"🔊 **[{target.first_name}](tg://user?id={target.id}) unmute kar diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "warn":
                count = add_warn(target.id, chat.id, f"Warned by {admin_user.first_name}")
                await msg.reply_text(f"⚠️ **[{target.first_name}](tg://user?id={target.id}) warned! (Total: {count})**", parse_mode=ParseMode.MARKDOWN)
                ...
                await msg.reply_text(f"⭐ **[{target.first_name}](tg://user?id={target.id}) admin bana diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "demote":
                await context.bot.promote_chat_member(chat.id, target.id, can_manage_chat=False)
                await msg.reply_text(f"⬇️ **[{target.first_name}](tg://user?id={target.id}) demote kar diya gaya!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "escrow_verify":
                conn = db_connect()
                conn.execute("UPDATE users SET escrow_verified=1 WHERE user_id=?", (target.id,))
                conn.commit(); conn.close()
                await msg.reply_text(f"💎 **[{target.first_name}](tg://user?id={target.id}) escrow-verified!**", parse_mode=ParseMode.MARKDOWN)

            elif action == "upgrade_trust":
                lvl = min(3, get_trust_level(target.id) + 1)
                set_trust_level(target.id, lvl)
                await msg.reply_text(
                    f"⭐ **[{target.first_name}](tg://user?id={target.id}) upgrade kar diya gaya — "
                ...
                set_trust_level(target.id, lvl)
                await msg.reply_text(f"⬇️ **[{target.first_name}](tg://user?id={target.id}) downgrade kar diya gaya.**", parse_mode=ParseMode.MARKDOWN)
...
async def send_admin_log_dm(context, admin_user, action, target):
    try:
        await context.bot.send_message(
            OWNER_ID,
            f"📋 **Admin Log**\n"
            f"👮 Admin: [{admin_user.first_name}](tg://user?id={admin_user.id})\n"
            f"⚡ Action: `{action}`\n"
            f"🎯 Target: [{target.first_name}](tg://user?id={target.id})\n"
...
async def trusted_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        lines.append(f"• {badge} — [{r['first_name']}](tg://user?id={r['user_id']}) ({uname})")
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_channels":
        ...
        text += "\n".join(f"• @{c}" for c in channels) if channels else "_Koi channel set nahi hai._"
        ...
        await query.edit_message_text(f"✅ **@{uname} remove kar diya gaya.**", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_markup())
        ...
        if not rows:
            ...
            text = "🏆 **Trusted Traders (All Groups)**\n\n" + "\n".join(
                f"• {TRUST_LEVELS.get(r['trust_level'])} — {r['first_name']} (`{r['user_id']}`)" for r in rows
            )
