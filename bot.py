...
ACTION_PERMISSION_REQUIREMENT = {
    ...
    "toggle_permission": "can_change_info",
    ...
    try:
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            if not perm_key:
                ...
                await context.bot.set_chat_permissions(chat.id, ChatPermissions(**new_perms_kwargs))
                perm_label = str(result.get("permission")).replace("_", " ").title()
                await msg.reply_text(
                    stylize(f"✅ <b>{perm_label}</b> ab is group me <b>{'ALLOWED' if enabled else 'BLOCKED'}</b> hai — "
                    f"baaki saari settings waisi hi rahi jaisi thi."),
                    parse_mode=ParseMode.HTML,
                )
