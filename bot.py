...
def db_export_json() -> str:
    ...
    for table in [
        ...
        "approved_groups", "known_groups", "promoted_by",
    ]:
...
def db_import_json(raw: str) -> tuple[bool, str]:
    ...
    try:
        conn = db_connect()
        cur = conn.cursor()
        for t in required_tables + [t for t in ("group_access", "group_settings", "filters", "custom_emojis", "approved_groups", "known_groups", "promoted_by") if t in data]:
...
def get_group_owner_contact(chat_id):
    ...
    if row:
        ...
        return label, f"tg://openmessage?user_id={row['user_id']}"
    conn.close()
...
jaise "2 warning do"; {WARN_BAN_LIMIT} total warnings pe user AUTO-BAN ho jaata
hai), unwarn (count ke saath, ya "saari hata do"), promote, demote, pin, unpin,
        ...
        if (now - last_notice) > DEAL_NOTICE_COOLDOWN_SECONDS:
            ...
            for r in verified_rows:
                if r["username"]:
                    ...
                    user_lines.append(f"• <a href='tg://openmessage?user_id={r['user_id']}'>{md_escape(r['first_name'])}</a>")
...
async def admins_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in members:
        u = m.user
        link = f"<a href='tg://openmessage?user_id={u.id}'>{md_escape(u.first_name)}</a>"
