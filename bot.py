...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user, command_text: str = "", capture: list = None):
    ...
    try:
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            id_chunks = [message_ids[i:i + 100] for i in range(0, len(message_ids), 100)]
            deleted = 0
            # No sender filtering anywhere below -- user, admin, owner, or
            # bot, every id in the range gets targeted the same way.
            for chunk in id_chunks:
                try:
                    await context.bot.delete_messages(chat.id, chunk)
                    deleted += len(chunk)
                except TelegramError as e:
                    # Whole-chunk delete failed (e.g. one id in it is already
                    # gone/too old) -- fall back to one-by-one for just this
                    # chunk so a single bad id doesn't cost the rest of it.
                    logger.warning("Purge: bulk delete failed for a chunk, retrying individually: %s", e)
                    for mid in chunk:
                        try:
                            await context.bot.delete_message(chat.id, mid)
                            deleted += 1
                        except TelegramError:
                            pass  # already gone / never existed -- skip and keep going
                await asyncio.sleep(0.1)  # small gap between chunks to stay under flood limits
