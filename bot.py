...
def _utf16_to_str_index_map(s: str):
    """idx[i] = python string index for UTF-16 code-unit position i."""
    mapping = []
...
def render_html_to_entities(html_text: str, emoji_map: dict):
    """Renders our small HTML subset (<b>, <code>, <a href>) PLUS {name}
    custom/premium-emoji tokens into (plain_text, entities) for sending
    WITHOUT parse_mode (Telegram's Bot API doesn't allow parse_mode and
    entities together, so any message using a real custom_emoji entity
    has to be built this way instead of the usual stylize()+parse_mode=HTML)."""
    if not html_text:
        return "", []
    ...
    def emit_text(chunk, apply_fancy):
        nonlocal utf16_pos
        last = 0
        for m in re.finditer(r"\{([\w\u0900-\u097F]+)\}", chunk):
            ...
            if name in emoji_map:
                placeholder_char, custom_emoji_id = emoji_map[name]
                out.append(placeholder_char)
                elen = _utf16_len(placeholder_char)
                entities.append(MessageEntity(type=MessageEntity.CUSTOM_EMOJI, offset=utf16_pos, length=elen, custom_emoji_id=custom_emoji_id))
                utf16_pos += elen
            else:
    ...
    for member in update.message.new_chat_members:
        ...
        if gsettings.get("welcome_text"):
            try:
                ...
                # placeholders, PLUS any saved custom/premium emoji {label} tokens.
                caption = gsettings["welcome_text"].format(
        ...
        emoji_map = await asyncio.to_thread(get_custom_emoji_map)
        final_text, entities = render_html_to_entities(caption, emoji_map)
        ...
        try:
            if photo_bytes:
                await context.bot.send_photo(
                    chat.id, photo=bytes(photo_bytes), caption=final_text, caption_entities=entities,
                    reply_markup=markup,
                )
            else:
                await context.bot.send_message(
                    chat.id, final_text, entities=entities,
...
async def send_filter_reply(context, chat_id, matched_filter, reply_to_message_id):
    try:
        ...
        if ftype == "photo":
            ...
            emoji_map = await asyncio.to_thread(get_custom_emoji_map)
            final_text, entities = render_html_to_entities(matched_filter["reply_text"], emoji_map)
            await context.bot.send_message(chat_id, final_text, entities=entities, reply_to_message_id=reply_to_message_id)
    except TelegramError:
        pass
...
async def emojis_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    emoji_map = await asyncio.to_thread(get_custom_emoji_map)
    final_text, entities = render_html_to_entities("\n".join(html_lines), emoji_map)
    await context.bot.send_message(update.effective_chat.id, final_text, entities=entities)
...
async def maybe_handle_custom_emoji_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    for e in custom_ents:
        ...
        placeholder_char = text[str_start:str_end]
        ...
        if not name:
            n = await asyncio.to_thread(count_custom_emojis)
            name = f"emoji{n + 1}"
        elif await asyncio.to_thread(name_taken_by_other, name, e.custom_emoji_id):
            n = await asyncio.to_thread(count_custom_emojis)
            name = f"{name}{n + 1}"

        await asyncio.to_thread(save_custom_emoji, e.custom_emoji_id, placeholder_char, name, update.effective_user.id)
        lines.append(
    ...
    emoji_map = await asyncio.to_thread(get_custom_emoji_map)
    final_text, out_entities = render_html_to_entities("\n\n".join(lines), emoji_map)
    await context.bot.send_message(update.effective_chat.id, final_text, entities=out_entities)
    return True
...
async def dm_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private":
        return
    if await maybe_handle_custom_emoji_save(update, context):
        return
    user_id = update.effective_user.id
