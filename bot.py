...
class ResolvedUser:
    """Lightweight stand-in for a Telegram User/Chat object, built purely
    from OUR OWN database. Has the same .id/.first_name/.username attributes
    that the rest of the code already expects from target.* -- so it works
    as a drop-in target wherever msg.reply_to_message.from_user is used."""
...
def find_user_by_username(username: str):
    """Resolve '@username' -> a target user using OUR bot's own database
    (every user who has ever sent a message gets upserted with their
    username). This is far more reliable than Telegram's get_chat(username)
    lookup, which frequently fails for regular user accounts even when the
    bot has definitely seen that user before in the same group."""
...
async def ai_check_link_permission(message_text: str, recent_context: str) -> dict:
    """
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message_has_link_pattern(text):
        ...
        result = await ai_check_link_permission(text, context_str)
        if result.get("is_link_or_promo") and not result.get("was_requested_by_admin"):
            ...
            try:
                await context.bot.send_message(
                    chat.id,
                    stylize(f"🚫 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, <b>promotion mat karo "
...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user, command_text: str = ""):
    ...
    if not target and command_text:
        # No reply given -- try resolving a "@username" mentioned in the
        # command text itself (e.g. "@lrvs12 isko admin banao..").
        # 1) First try OUR OWN database (fast, reliable -- works for anyone
        #    who has ever sent a message in the group).
        # 2) Only if that fails, fall back to asking Telegram directly
        #    (works for public usernames Telegram itself can resolve).
        uname_match = re.search(r"@([A-Za-z0-9_]{4,})", command_text)
        if uname_match:
            target = find_user_by_username(uname_match.group(1))
            if not target:
                try:
                    target = await context.bot.get_chat(f"@{uname_match.group(1)}")
                except TelegramError:
                    target = None
    if target:
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if not target:
                await msg.reply_text(
                    stylize("↩️ <b>Kisi user ke message ko reply karo, ya @username likh kar batao.</b>"),
                    parse_mode=ParseMode.HTML,
                )
                return
            ...
            if action == "ban":
                ...
                set_trust_level(target.id, lvl)
                await msg.reply_text(stylize(f"⬇️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> downgrade kar diya gaya.</b>"), parse_mode=ParseMode.HTML)
        ...
        await msg.reply_text(stylize(f"❌ <b>Telegram error: {e.message}</b>\n(Shayad bot ke paas woh permission nahi hai)"), parse_mode=ParseMode.HTML)
    except TelegramError as e:
        await msg.reply_text(stylize(f"❌ <b>Error: {e}</b>"), parse_mode=ParseMode.HTML)
