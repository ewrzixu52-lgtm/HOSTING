...
def db_export_json() -> str:
    ...
    for table in [
        ...
        "group_access", "group_settings", "filters",
    ]:
...
def db_import_json(raw: str) -> tuple[bool, str]:
    ...
    try:
        conn = db_connect()
        cur = conn.cursor()
        for t in required_tables + [t for t in ("group_access", "group_settings", "filters") if t in data]:
...
async def handle_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        ...
        if gsettings.get("welcome_text"):
            # Custom per-group welcome text -- supports {name}/{group}/{id}/{username} placeholders.
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if matched_filter:
        try:
            ...
            if ftype == "photo":
                await context.bot.send_photo(chat.id, matched_filter["file_id"], caption=matched_filter["reply_text"] or None, reply_to_message_id=msg.message_id)
            elif ftype == "sticker":
                await context.bot.send_sticker(chat.id, matched_filter["file_id"], reply_to_message_id=msg.message_id)
            elif ftype == "animation":
                await context.bot.send_animation(chat.id, matched_filter["file_id"], caption=matched_filter["reply_text"] or None, reply_to_message_id=msg.message_id)
            elif ftype == "video":
                await context.bot.send_video(chat.id, matched_filter["file_id"], caption=matched_filter["reply_text"] or None, reply_to_message_id=msg.message_id)
            elif ftype == "document":
                await context.bot.send_document(chat.id, matched_filter["file_id"], caption=matched_filter["reply_text"] or None, reply_to_message_id=msg.message_id)
            elif matched_filter["reply_text"]:
                await msg.reply_text(stylize(matched_filter["reply_text"]), parse_mode=ParseMode.HTML)
        except TelegramError:
            pass
        return
