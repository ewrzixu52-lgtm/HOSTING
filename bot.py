...
def _refresh_custom_emoji_cache():
    ...
    after every save/rename/delete -- keeps stylize() DB-free on the hot path."""
...
def _apply_custom_emojis(text: str) -> str:
    """Single regex pass: replaces {name} tokens with the matching saved
    premium emoji, AND auto-replaces any plain emoji character that has a
    saved premium alternate -- both via Telegram's native <tg-emoji
    emoji-id="..."> HTML tag (works inside parse_mode=HTML same as <b>/<a>)."""
    if not _custom_emoji_cache_by_name and not _custom_emoji_cache_by_char:
        return text
    if _custom_emoji_cache_by_char:
        char_alt = "|".join(re.escape(c) for c in _custom_emoji_cache_by_char.keys())
        pattern = re.compile(f"{_EMOJI_NAME_TOKEN_RE}|(?P<echar>{char_alt})")
    else:
        pattern = re.compile(_EMOJI_NAME_TOKEN_RE)
    ...
    def repl(m):
        ...
        if gd.get("ename"):
            ...
            if key in _custom_emoji_cache_by_name:
                char, eid = _custom_emoji_cache_by_name[key]
                return f'<tg-emoji emoji-id="{eid}">{char}</tg-emoji>'
            return m.group(0)
        echar = gd.get("echar")
        if echar and echar in _custom_emoji_cache_by_char:
            _, eid = _custom_emoji_cache_by_char[echar]
            return f'<tg-emoji emoji-id="{eid}">{echar}</tg-emoji>'
        return m.group(0)
...
def stylize(html_text: str) -> str:
    ...
    Also auto-applies any saved premium/custom emoji (see _apply_custom_emojis)."""
    if not html_text:
        return html_text
    ...
    for tok in tokens:
        if tok.startswith("<"):
            ...
            piece = tok if skip_mode else fancy(tok)
            result.append(_apply_custom_emojis(piece))
    return "".join(result)
...
async def send_filter_reply(context, chat_id, matched_filter, reply_to_message_id):
    try:
        ...
        if ftype == "photo":
            ...
            await context.bot.send_message(chat_id, stylize(matched_filter["reply_text"]), parse_mode=ParseMode.HTML, reply_to_message_id=reply_to_message_id)
    except TelegramError:
        pass
...
async def emojis_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(update.effective_chat.id, stylize("\n".join(html_lines)), parse_mode=ParseMode.HTML)
