...
# NOTE: bare "@username" was intentionally removed from this pattern.
# Two normal members tagging each other ("@ravi dekh isko") was matching
# here and firing an AI call every single time -- wasting the free-tier
# Groq quota on completely normal chat. Only an actual link (http/https,
# t.me/, or a real dot-domain like "example.com") now triggers the AI
# link-permission check. Escrow-impersonation @mentions are still caught
# separately by extract_owner_username_mentions() in the escrow guard
# section, which is unaffected by this change.
