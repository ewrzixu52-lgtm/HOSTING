...
_MANAGER_ROAST_FALLBACKS = [
    "🙄 <b>{name}, admin bhi nahi hai, manager ko awaaz laga raha hai jaise malik ho.</b>",
    "😹 <b>{name} ka VIP pass expire ho chuka hai, member hi hai tu.</b>",
    "🤡 <b>Itni entitlement {name} mein bhi kaha se aayi, permission toh hai nahi.</b>",
    "😏 <b>{name}, manager tere liye free nahi hai, admin ban ke aa pehle.</b>",
]


async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    """A regular (non-admin) member called for 'manager' -- generate a
    fresh, savage, comedic Hinglish roast every time, never a static
    template. Cutting and sharp, but no slurs/hate speech/real threats."""
    system_prompt = (
        "Tum ek savage, sarcastic, comedic Telegram group bot ho. Ek NORMAL member "
        "(admin nahi) ne 'manager' ko bulaya/pucha hai. Use ekdum tikha aur savage "
        "roast do -- itna sharp ki padhke thoda chidh jaaye -- 1-2 punchy lines me, "
        "Hinglish me (jaise: khud ko VIP samajh raha hai, entitled hai, permission "
        "zero hai attitude full hai, etc). Har roast UNIQUE hona chahiye -- kabhi bhi "
        "ek jaisi template mat dohrao. Sirf itna avoid karo: slurs, gaali-galoch ke "
        "asli bure/abusive words, aur kisi real threat jaisa kuch -- baaki jitna "
        "tikha/savage ho sake utna acha, halka-fulka mat rakho. Aakhir me halke se "
        "poochho ki 'admin bhi nahi hai fir kya kaam hai?' jaisa kuch, apne style me."
    )
    user_content = f"User '{first_name}' ne yeh message bheja: \"{message_text}\""
    raw = await asyncio.to_thread(
        _grok_call_raw,
        [
            {"role": "system", "content": system_prompt},
    ...
    if not raw:
        return random.choice(_MANAGER_ROAST_FALLBACKS).format(name=md_escape(first_name))
    return raw.strip()
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if re.search(r"\b(manager|मैनेजर)\b", text, re.IGNORECASE) and not text.strip().endswith("..") and not text.strip().endswith("##"):
        if admin_ok:
            try:
                await msg.reply_text(stylize("🫡 <b>Ji boss! Hukum kariye, kya kaam hai?</b>"), parse_mode=ParseMode.HTML)
            except TelegramError:
                pass
        else:
            roast = await ai_roast_manager_caller(text, user.first_name)
            try:
                await msg.reply_text(stylize(roast), parse_mode=ParseMode.HTML)
            except TelegramError:
                pass
        return
    ...
    if text.strip().endswith(".."):
        if not admin_ok:
            roast = random.choice(_NOT_ADMIN_ROASTS)
            try:
                markup = await owner_credit_button(context)
                await context.bot.send_message(
                    chat.id,
                    stylize(f"{roast} <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a> 😹"),
                    parse_mode=ParseMode.HTML,
                    reply_markup=markup,
                )
            except TelegramError:
...
# Rotates so it's not the same line every time.
_NOT_ADMIN_ROASTS = [
    "😂 <b>Admin ka sapna abhi sapna hi rahega,</b>",
    "🙄 <b>Itni himmat? Member ho, boss nahi,</b>",
    "🤡 <b>Command chalane se pehle power toh le aa,</b>",
    "😹 <b>Group ka malik samajh liya apne aap ko kya,</b>",
    "😏 <b>Permission zero hai bhai, attitude full hai,</b>",
]
