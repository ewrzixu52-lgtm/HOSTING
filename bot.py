...
async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    ...
    runs normally."""
    system_prompt = (
        "You are an unapologetic, brutally witty, and extremely savage roast bot "
        "in a group chat. Your only goal is to deliver razor-sharp, hilarious "
        "roasts that instantly burn the user and make the whole group laugh.\n\n"
        "Rules:\n"
        "1. Tone: Highly sarcastic, witty, confident, and unapologetic. Zero fake "
        "politeness, zero sweet talk.\n"
        "2. Language: Hinglish ONLY -- Hindi ki tarah sunna chahiye par likha "
        "Roman/English script mein ho, bilkul jaisa log WhatsApp pe chat karte hain "
        "(Devanagari script kabhi mat use karna). Natural, modern Indian slang, "
        "colloquial punchlines, witty comparisons.\n"
        "3. Content: Attack their logic, ego, message context, typing style, or "
        "common silly habits. Do NOT give generic, soft jokes (avoid dad jokes or "
        "schoolyard rhymes). Make it feel personal to what they just said. Strictly "
        "avoid slurs, casteist/religious attacks, or hate speech -- the burn must "
        "come purely from superior sarcasm, logic destruction, and witty insults.\n"
        "4. Length: Keep it punchy (1 to 3 sentences max). Long paragraphs ruin the "
        "timing of a roast.\n"
        "5. Objective: Leave them with no comeback. Maximum embarrassment, maximum "
        "humor.\n\n"
        "Context: this is a NORMAL member (not an admin) calling/asking for the "
        "'manager' as if they have any authority here -- that's exactly what you're "
        "roasting them for. Use their first name naturally in the roast. You may "
        "use 1-2 plain/normal emoji (not custom or made-up ones) if it fits, don't "
        "overload it with emoji."
    )
...
- Naye member ko photo/video + welcome message + escrow disclaimer + channel
  buttons.
- Gaali dene par: message delete + {ABUSE_MUTE_MIN} min mute + warning.
- Link/username/promotion bhejne par: message delete + warning, instant
  ...
  links/domains pakde jaate hain).
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if action == "ban":
                ...
                if verify_member.status != ChatMemberStatus.ADMINISTRATOR:
                    ...
                    await _reply(
                        stylize(f"⭐ <b>Verified:</b> <a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> "
                        f"<b>ab admin hai</b> — default: Delete messages, Pin messages, Invite users, Warn members "
                        f"(mute/ban/kick nahi kar sakta).\n\n"
                        f"Extra chahiye? Reply/@username + number bolo (jaise \"isko 2,5 do\"). Poori list ## se poochho."),
                        parse_mode=ParseMode.HTML,
                    )
...
async def rules_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rules:
        await query.answer("Koi rules set nahi hain.", show_alert=True)
        return
    try:
        ...
        await query.answer("Rules aapko DM me bhej di gayi hain!")
    except TelegramError:
        await query.answer("Pehle bot ko DM me /start karo, phir rules dekh paoge.", show_alert=True)
