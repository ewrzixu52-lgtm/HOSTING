...
def ai_owner_chat(question: str, user_data_context: str = "") -> str:
        ...
        try:
            if photo_bytes:
                ...
                await context.bot.send_message(
                    chat.id, caption, parse_mode=ParseMode.HTML,
...
async def check_impersonation(context: ContextTypes.DEFAULT_TYPE, chat_id, member, photo_bytes):
    ...
    if overall >= 50:
        ...
        try:
            await context.bot.send_message(
                chat_id,
                f"⚠️ <a href='tg://user?id={member.id}'>{md_escape(member.first_name)}</a> <b>ka profile owner se "
    ...
    for chat_id, active in list(_raid_active.items()):
        ...
        if now - last > RAID_COOLDOWN_GAP:
            ...
            try:
                await context.bot.send_message(
                    chat_id,
                    "🛡️ <b>Fake members detected & removed.</b>\n"
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(dq) >= ANTIFLOOD_MAX_MSGS:
        ...
        try:
            await context.bot.send_message(
                chat.id,
                f"🚫 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a> <b>spam kar raha tha — "
    ...
    if last and last[0] == text and text.strip() and (now - last[1]) < DUPLICATE_WINDOW:
        try:
            await msg.delete()
            await context.bot.send_message(chat.id, "🛑 <b>Ruko zara, sabar karo!</b> 😤")
        except TelegramError:
            pass
        return
    ...
    if contains_abuse(text):
        ...
        try:
            await context.bot.send_message(
                chat.id,
                f"🤬 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a> <b>ne gaali di — "
    ...
    if "escrow" in text.lower():
        ...
        if fake_mention:
            ...
            if result.get("is_impersonation"):
                ...
                await context.bot.send_message(
                    chat.id,
                    "⚠️ <b>Kisi aur escrow worker ke paas mat jaayein.</b>\n"
    ...
    if message_has_link_pattern(text):
        ...
        if result.get("is_link_or_promo") and not result.get("was_requested_by_admin"):
            ...
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🚫 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, <b>promotion mat karo "
    ...
    if text.strip().endswith("##"):
        if not is_owner(user.id):
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🔒 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, "
        ...
        answer = ai_owner_chat(question, user_data_context)
        try:
            await msg.reply_text(answer)  # plain text -- AI's own answer, no HTML parse mode needed
        except TelegramError:
            pass
        return
    ...
    if text.strip().endswith(".."):
        ...
        if not admin_ok:
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🔒 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, <b>aap admin nahi ho.</b>",
                    parse_mode=ParseMode.HTML,
                )
            except TelegramError:
                pass
    ...
    if message_has_link_pattern(text):
        ...
        if result.get("is_link_or_promo") and not result.get("was_requested_by_admin"):
            ...
            try:
                await context.bot.send_message(
                    chat.id,
                    f"🚫 <a href='tg://user?id={user.id}'>{md_escape(user.first_name)}</a>, <b>message edit karke "
...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user, command_text: str = ""):
    ...
    if action == "unknown" or not action:
        ...
        if target_for_question:
            conn = db_connect()
            row = conn.execute("SELECT * FROM users WHERE user_id=?", (target_for_question.id,)).fetchone()
            conn.close()
            warns = get_warn_count(target_for_question.id, chat.id)
            trust = row["trust_level"] if row else 0
            escrow = "Yes" if (row and row["escrow_verified"]) else "No"
            join_date = row["join_date"][:10] if row and row["join_date"] else "N/A"
            user_data_context = (
        ...
        answer = ai_owner_chat(command_text or "?", user_data_context)
        await msg.reply_text(answer)
        return

    if action in OWNER_ONLY_ACTIONS and not is_owner(admin_user.id):
        await msg.reply_text("🔒 <b>Yeh command sirf owner use kar sakta hai.</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if target:
        ...
        upsert_user(target.id, target.username, target.first_name, chat.id)
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if not target:
                await msg.reply_text(
                    "↩️ <b>Kisi user ke message ko reply karke command do.</b>",
                    parse_mode=ParseMode.HTML,
                )
                return
            ...
            if action == "ban":
                ...
                await msg.reply_text(f"🔨 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ban kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "unban":
                await context.bot.unban_chat_member(chat.id, target.id, only_if_banned=True)
                remove_ban_record(target.id, chat.id)
                await msg.reply_text(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> unban kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "kick":
                await context.bot.ban_chat_member(chat.id, target.id)
                await context.bot.unban_chat_member(chat.id, target.id)
                await msg.reply_text(f"👢 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> kick kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "mute":
                mins = duration or 60
                await mute_user(context, chat.id, target.id, mins)
                await msg.reply_text(f"🔇 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> {mins} min ke liye mute!</b>", parse_mode=ParseMode.HTML)

            elif action == "unmute":
                await unmute_user(context, chat.id, target.id)
                await msg.reply_text(f"🔊 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> unmute kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "warn":
                # req_count: kitni warnings ek saath deni hain (default 1)
                add_count = 999 if req_count >= 999 else req_count
                count = get_warn_count(target.id, chat.id)
                ...
                await msg.reply_text(f"⚠️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ko {label} diye gaye! (Total: {count})</b>", parse_mode=ParseMode.HTML)

            elif action == "unwarn":
                current_count = get_warn_count(target.id, chat.id)
                if current_count == 0:
                    await msg.reply_text(
                        f"ℹ️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ki koi warning hai hi nahi, hataane ko kuch nahi hai.</b>",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    ...
                    conn.commit()
                    conn.close()
                    new_count = get_warn_count(target.id, chat.id)
                    label = f"{to_remove} warnings" if to_remove > 1 else "1 warning"
                    await msg.reply_text(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ki {label} hata di gayi! (Baaki: {new_count})</b>", parse_mode=ParseMode.HTML)

            elif action == "pin":
                if not msg.reply_to_message:
                    await msg.reply_text("↩️ <b>Jo message pin karna hai, use reply karke command do.</b>", parse_mode=ParseMode.HTML)
                else:
                    await context.bot.pin_chat_message(chat.id, msg.reply_to_message.message_id)
                    await msg.reply_text("📌 <b>Message pin kar diya gaya!</b>", parse_mode=ParseMode.HTML)
                ...
                await msg.reply_text("📌 <b>Unpin kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "delete":
                if not msg.reply_to_message:
                    await msg.reply_text("↩️ <b>Jo message delete karna hai, use reply karke command do.</b>", parse_mode=ParseMode.HTML)
                else:
                    await context.bot.delete_message(chat.id, msg.reply_to_message.message_id)
                    await msg.reply_text("🗑️ <b>Message delete kar diya gaya!</b>", parse_mode=ParseMode.HTML)
                ...
                await msg.reply_text(f"⭐ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> admin bana diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "demote":
                await context.bot.promote_chat_member(chat.id, target.id, can_manage_chat=False)
                await msg.reply_text(f"⬇️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> demote kar diya gaya!</b>", parse_mode=ParseMode.HTML)

            elif action == "escrow_verify":
                conn = db_connect()
                conn.execute("UPDATE users SET escrow_verified=1 WHERE user_id=?", (target.id,))
                conn.commit(); conn.close()
                await msg.reply_text(f"💎 <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> escrow-verified!</b>", parse_mode=ParseMode.HTML)

            elif action == "escrow_unverify":
                conn = db_connect()
                conn.execute("UPDATE users SET escrow_verified=0 WHERE user_id=?", (target.id,))
                conn.commit(); conn.close()
                await msg.reply_text(f"❌ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ka escrow-verified badge hata diya gaya.</b>", parse_mode=ParseMode.HTML)

            elif action == "upgrade_trust":
                lvl = min(3, get_trust_level(target.id) + 1)
                set_trust_level(target.id, lvl)
                await msg.reply_text(
                    f"⭐ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> upgrade kar diya gaya — "
                ...
                set_trust_level(target.id, lvl)
                await msg.reply_text(f"⬇️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> downgrade kar diya gaya.</b>", parse_mode=ParseMode.HTML)
                ...
                await msg.reply_text(
                    "🧊 <b>Dispute freeze kar diya gaya hai jab tak owner resolve nahi karta.</b>",
                    parse_mode=ParseMode.HTML,
                )

            elif action == "dispute_unfreeze":
                _frozen_disputes[chat.id].discard(target.id)
                await msg.reply_text("✅ <b>Dispute unfreeze kar diya gaya.</b>", parse_mode=ParseMode.HTML)
                ...
                answer = ai_owner_chat(command_text or "Iski info do", user_data_context)
                await msg.reply_text(answer)

            elif action == "set_title":
                if target.id == OWNER_ID:
                    await msg.reply_text(
                        "🔒 <b>Apna khud ka tag/title change nahi kar sakte — sirf doosron ka.</b>",
                        parse_mode=ParseMode.HTML,
                    )
                elif not title_text:
                    await msg.reply_text(
                        "🤔 <b>Naya title kya rakhna hai woh clearly bolo.</b>",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    ...
                    try:
                        ...
                        await msg.reply_text(
                            f"🏷️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ka tag "
                            f"'{md_escape(new_title)}' set kar diya gaya!</b>",
                            parse_mode=ParseMode.HTML,
                        )
                    except BadRequest:
                        await msg.reply_text(
                            "❌ <b>Yeh sirf admins ke liye kaam karta hai — Telegram ka apna rule hai.</b>\n"
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            if not msg.reply_to_message:
                await msg.reply_text("↩️ <b>Jahan tak purge karna hai us message ko reply karo.</b>", parse_mode=ParseMode.HTML)
                return
            ...
            await context.bot.send_message(chat.id, "🔒 <b>Chat lock kar diya gaya — sirf admins bhej sakte hain.</b>", parse_mode=ParseMode.HTML)
            ...
            await context.bot.send_message(chat.id, "🔓 <b>Chat unlock kar diya gaya!</b>", parse_mode=ParseMode.HTML)
            log_admin_action(admin_user.id, admin_user.first_name, "unlock", 0, "")

    except BadRequest as e:
        await msg.reply_text(f"❌ <b>Telegram error: {e.message}</b>\n(Shayad bot ke paas woh permission nahi hai)", parse_mode=ParseMode.HTML)
    except TelegramError as e:
        await msg.reply_text(f"❌ <b>Error: {e}</b>", parse_mode=ParseMode.HTML)
...
async def user_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = update.effective_message
    if not msg.reply_to_message:
        await msg.reply_text("↩️ <b>Kisi user ke message ko reply karke /info bhejo.</b>", parse_mode=ParseMode.HTML)
        return
...
async def trusted_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await update.effective_message.reply_text("😶 <b>Abhi tak koi Trusted Trader nahi hai.</b>", parse_mode=ParseMode.HTML)
        return
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        await update.effective_message.reply_text("👋 <b>Namaste! Group management ke liye taiyar hoon.</b>", parse_mode=ParseMode.HTML)
        return
    if update.effective_user.id != OWNER_ID:
        await update.effective_message.reply_text("🔒 <b>Yeh admin panel sirf owner ke liye hai.</b>", parse_mode=ParseMode.HTML)
        return
    await update.effective_message.reply_text(
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    if update.effective_user.id != OWNER_ID:
        await query.edit_message_text("🔒 <b>Sirf owner use kar sakta hai.</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if data == "panel_channels":
        ...
        await query.edit_message_text(f"✅ <b>@{md_escape(uname)} remove kar diya gaya.</b>", parse_mode=ParseMode.HTML, reply_markup=admin_panel_markup())
...
async def dm_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        f"✅ <b>@{text} welcome-channel list me add ho gaya!</b>",
...
async def dm_document_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("❌ <b>File read nahi ho payi.</b>", parse_mode=ParseMode.HTML)
        return
    ok, message_text = db_import_json(raw_text)
    context.user_data["panel_state"] = None
    await update.effective_message.reply_text(message_text, parse_mode=ParseMode.HTML, reply_markup=admin_panel_markup())
...
def main():
    ...
    # handler above so the bot never silently dies. Logs it fully AND
    # (best-effort) DMs the owner a short alert, so problems get noticed
    # immediately instead of the bot just going quiet.
