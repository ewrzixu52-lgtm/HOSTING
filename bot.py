...
_MANAGER_ROASTS = [
    ["😂 <b>{name} khud ko CEO samajhta hai, hakikat mein unpaid intern hai.</b>", "8", "Ego bada, post koi nahi"],
    ["🤡 <b>{name}, himmat dekho — zero power, full boss attitude.</b>", "7", "Power se zyada attitude"],
    ["🙄 <b>{name}, tu manager nahi, is group ka background extra hai.</b>", "6", "Role clear kar diya"],
    ["😹 <b>{name} khud ko boss samajh raha, sab peeche se hass rahe hain.</b>", "7", "Sabko pata, {name} ko nahi"],
    ["💀 <b>{name}, admin panel dekha bhi nahi, order kaise de raha?</b>", "8", "Bina access ke hukum"],
    ["😏 <b>{name} mein itni entitlement kahan se, resume mein 'admin' likha kya?</b>", "6", "Fake designation pakda gaya"],
    ["🤣 <b>{name}, naukar hai kya tu uska? Member hai, haq mat jata.</b>", "7", "Seedha reality check"],
    ["😆 <b>{name}, tu group mein utna hi important jitna 'last seen recently'.</b>", "6", "Invisible level importance"],
    ["🙃 <b>{name} ki permission list khaali, ego full tank hai.</b>", "7", "Permission zero, drama full"],
    ["🫠 <b>{name}, teri authority ka level bilkul 'guest' jaisa hai.</b>", "5", "Guest pass, boss wala sapna"],
    ["😹 <b>{name} ne dimaag mein khud ko promote kar liya, record mein member hi hai.</b>", "8", "Self-promote scam pakda"],
]

_last_manager_roast_fallback = None  # index of the last fallback set used -- never repeat it back-to-back
...
async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    ...
    actually typed, in a ranked '2-3 roasts scored by savageness' format.
    Falls back to the static pool ONLY if the AI call itself fails (no API
    key / network / rate limit) -- the fallback explicitly excludes
    whichever set it used last time, so even several failed-AI-calls in a
    row can never repeat back-to-back."""
    system_prompt = (
        ...
        "OUTPUT: write exactly 2 roasts, ranked by how savage they are, in "
        "EXACTLY this format (plain text, no markdown symbols):\n"
        "💀 SABSE KHATARNAK (Rank 1) — Score: X/10\n"
        "[roast line]\n"
        "Kyun: [one short reason]\n\n"
        "🔥 Rank 2 — Score: X/10\n"
        "[roast line]\n"
        "Kyun: [one short reason]\n\n"
        "RULES:\n"
        "1. Each roast line: ONE short punchy WhatsApp-style line, 5-12 words "
        "max. No long sentences.\n"
        ...
        "3. Every roast must reference their name directly or through a clever "
        "pun/wordplay on it -- make it feel personal, not generic.\n"
        "4. Build each roast around ONE clever, concrete, unexpected "
        "comparison (apps, everyday digital-life stuff, common habits) -- "
        "show the joke, don't just state 'you have ego'.\n"
        ...
        "6. Max 1 emoji inside the roast line itself (the rank-header emoji "
        "above doesn't count).\n"
        "7. Every single roast, every single time, must use a genuinely "
        "different angle -- never repeat a comparison you've used before.\n"
    ...
    if not raw:
        ...
        n = len(_MANAGER_ROASTS)
        # Pick 2 distinct lines for the fallback pair, avoiding whichever
        # pair (by starting index) was used last time.
        start_choices = list(range(n))
        if _last_manager_roast_fallback in start_choices and n > 2:
            start_choices.remove(_last_manager_roast_fallback)
        start = random.choice(start_choices)
        second = (start + random.randint(1, n - 1)) % n
        _last_manager_roast_fallback = start
        name = md_escape(first_name)
        pick1 = _MANAGER_ROASTS[start]
        pick2 = _MANAGER_ROASTS[second]
        return (
            f"💀 SABSE KHATARNAK (Rank 1) — Score: {pick1[1]}/10\n"
            f"{pick1[0].format(name=name)}\n"
            f"Kyun: {pick1[2].format(name=name)}\n\n"
            f"🔥 Rank 2 — Score: {pick2[1]}/10\n"
            f"{pick2[0].format(name=name)}\n"
            f"Kyun: {pick2[2].format(name=name)}"
        )
    return raw.strip()
...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user, command_text: str = "", capture: list = None):
    ...
    try:
        ...
        # ---- OWNER-ONLY, NON-USER-TARGETED ----
            ...
            deleted = 0
            # IMPORTANT: the bulk delete_messages() call does NOT raise when
            # some ids in the batch can't actually be deleted -- Telegram
            # just silently skips those and still reports success for the
            # whole batch. That was the real bug: we were counting
            # `len(chunk)` as deleted just because no exception came back,
            # so the reported number was fake and a chunk with a single bad
            # id left the rest of it undeleted with nothing to show for it.
            # Deleting one id at a time is the only way to know, per
            # message, whether it actually got deleted -- no sender
            # filtering here at all, user/admin/owner/bot are all targeted
            # the same way.
            for idx, mid in enumerate(message_ids):
                try:
                    ...
                    pass  # already gone / never existed / can't be deleted -- skip and keep going
                if (idx + 1) % 25 == 0:
                    await asyncio.sleep(1)  # brief pause every 25 deletes to stay under flood limits
