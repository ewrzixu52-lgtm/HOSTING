...
def db_import_json(raw: str) -> tuple[bool, str]:
    ...
    try:
        conn = db_connect()
        cur = conn.cursor()
        for t in required_tables + [t for t in ("group_access", "group_settings", "filters", "custom_emojis", "approved_groups", "known_groups") if t in data]:
