...
_MANAGER_ROASTS = [
    "😂 <b>{name} ko laga group ka CEO hai woh, asal mein sirf ek unpaid intern hai.</b>",
    "🤡 <b>{name}, himmat ki daad deni padegi — zero power, full main-character energy.</b>",
    "🙄 <b>Arre {name}, tu manager thodi hai, tu toh sirf group ka background character hai.</b>",
    "😹 <b>{name} khud ko boss samajhta hai, baaki sab members peeche se hass rahe hain.</b>",
    "💀 <b>{name}, admin panel dekh bhi nahi sakta, phir order kaise de raha hai?</b>",
    "😏 <b>Itni entitlement {name} mein bhari kahan se, resume mein 'admin' likh diya kya?</b>",
    "🤣 <b>{name} manager ko bula raha hai jaise uska naukar ho, member hai bas member.</b>",
    "😆 <b>{name}, tu group mein utna hi important hai jitna last seen 'recently' hota hai.</b>",
    "🙃 <b>{name} ki permission list khaali hai, ego full hai.</b>",
    "😂 <b>{name} soch raha hai manager uske liye free mein naachega, sapna dekhna band kar.</b>",
    "🫠 <b>{name}, teri authority ka level bilkul 'guest' jaisa hai — settle down.</b>",
    "😹 <b>{name} ne khud ko promote kar liya dimaag mein, group ke record mein toh member hi hai.</b>",
]
...
async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    ...
    so there's still variety even then, but the AI path is what actually
    runs normally."""
    system_prompt = (
    ...
    if not raw:
        return random.choice(_MANAGER_ROASTS).format(name=md_escape(first_name))
