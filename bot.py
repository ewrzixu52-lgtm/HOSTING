...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user):
    ...
    if action == "unknown" or not action:
        await msg.reply_text("🤔 <b>Samajh nahi aaya kya command hai — thoda clearly bolo.</b>", parse_mode=ParseMode.HTML)
        return
...
async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    never silently stops working. Always logs the full traceback, and
    (best-effort, never crashes if this itself fails) DMs the owner a short
    alert so problems get noticed right away instead of the bot going quiet."""
    logger.error("Unhandled exception while processing update:", exc_info=context.error)

    try:
        import traceback
        tb_string = "".join(traceback.format_exception(None, context.error, context.error.__traceback__))
        short_tb = tb_string[-500:]  # keep the DM short, last 500 chars is usually the useful part
        await context.bot.send_message(
            OWNER_ID,
            f"🚨 <b>Bot Error</b>\n\n<code>{md_escape(short_tb)}</code>",
            parse_mode=ParseMode.HTML,
        )
    except Exception:
