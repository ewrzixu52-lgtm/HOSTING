...
async def execute_admin_action(context: ContextTypes.DEFAULT_TYPE, update: Update, result: dict, admin_user):
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
                      ...
                      "dispute_unfreeze", "info", "pin", "unpin", "delete"):
            if not target:
            ...
            if action == "ban":
                ...
                await msg.reply_text(f"⚠️ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> warned! (Total: {count})</b>", parse_mode=ParseMode.HTML)

            elif action == "unwarn":
                conn = db_connect()
                last = conn.execute(
                    "SELECT id FROM warns WHERE user_id=? AND chat_id=? ORDER BY id DESC LIMIT 1",
                    (target.id, chat.id),
                ).fetchone()
                if last:
                    conn.execute("DELETE FROM warns WHERE id=?", (last["id"],))
                    conn.commit()
                conn.close()
                new_count = get_warn_count(target.id, chat.id)
                await msg.reply_text(f"✅ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ki 1 warning hata di gayi! (Baaki: {new_count})</b>", parse_mode=ParseMode.HTML)
