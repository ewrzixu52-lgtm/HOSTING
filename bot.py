...
async def ai_roast_manager_caller(message_text: str, first_name: str) -> str:
    ...
    fresh, witty, sarcastic Hinglish roast every time, never a static
    template. Playful, never abusive/profane."""
    system_prompt = (
        "Tum ek witty, sarcastic, comedic Telegram group bot ho. Ek NORMAL member "
        "(admin nahi) ne 'manager' ko bulaya/pucha hai. Use sarcastically aur funny "
        "andaaz me roast karo -- 1-2 punchy lines me, Hindi/Hinglish me (jaise: kyun "
        "bula raha hai, khud ko VIP samajh raha hai, entitled lag raha hai, etc). Har "
        "roast UNIQUE, sharp aur funny hona chahiye -- kabhi bhi ek jaisi template mat "
        "dohrao. Heavy abuse/gaali NAHI use karni, sirf halka-fulka roast, playful "
        "rehna hai. Aakhir me halke se poochho ki 'admin bhi nahi hai fir kya kaam hai?' "
        "jaisa kuch, apne style me."
    )
    ...
    if not raw:
        return f"🙄 <b>{md_escape(first_name)}, tu admin bhi nahi hai, manager ko kyun bula raha hai?</b>"
    return raw.strip()
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if re.search(r"\b(manager|मैनेजर)\b", text, re.IGNORECASE) and not text.strip().endswith("..") and not text.strip().endswith("##"):
        if admin_ok:
            try:
                await msg.reply_text(stylize("🫡 <b>जी बॉस! हुकुम करिए, क्या काम है?</b>"), parse_mode=ParseMode.HTML)
            except TelegramError:
                pass
        else:
    ...
    if text.strip().endswith(".."):
        ...
        # Resolve a display target for the header (best-effort -- same
        # resolution execute_admin_action does internally per-action).
        header_target = None
        if msg.reply_to_message:
        ...
        body = "\n".join(captured)
        final_text, final_entities = render_with_emojis(f"{header}\n{body}" if body else header)
        try:
