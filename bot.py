...
async def ai_classify_admin_command(command_text: str, has_reply_target: bool, recent_context: str = "") -> dict:
    ...
    system_prompt = (
        ...
        'Respond ONLY with compact JSON: {"reasoning": "...", "actions": '
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "escrow" in text.lower():
        ...
        if fake_mention:
            ...
            if result.get("is_impersonation"):
                ...
                final_text, final_entities = render_with_emojis(
                    f"⚠️ <b>Kisi aur escrow worker ke paas mat jaayein.</b>\n\n"
    ...
    if text.strip().endswith(".."):
        ...
        if len(actions_list) > 1:
            header_lines = ["📋 <b>Request:</b>\n"]
            for i, a in enumerate(actions_list, 1):
                header_lines.append(f"{i}. {str(a.get('action', 'unknown')).replace('_', ' ').title()}")
            final_text, final_entities = render_with_emojis("\n".join(header_lines))
            try:
                await context.bot.send_message(chat.id, final_text, entities=final_entities)
            except TelegramError:
    ...
    try:
        ...
        if action in ("ban", "unban", "kick", "mute", "unmute", "warn", "unwarn",
            ...
            if action == "ban":
                ...
                await msg.reply_text(
                    stylize(f"⭐ <b><a href='tg://user?id={target.id}'>{md_escape(target.first_name)}</a> ab admin hai</b> "
                    f"— default permissions: <b>Delete messages, Pin messages, Invite users via link, Warn "
                    f"members</b> (mute/ban/kick nahi kar sakta).\n\n"
                    f"Extra permissions dene ke liye, kabhi bhi is user ko reply karke ya uska @username "
                    f"leke, number(s) comma se bolo (jaise \"isko 2,5 permission do\"). Poori list ke liye "
                    f"koi bhi message ke aakhir me ## lagakar poocho."),
                    parse_mode=ParseMode.HTML,
                )
